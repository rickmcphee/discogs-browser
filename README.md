...
**Deployment / rate-limit keying:** the invite-redemption and Discogs-OAuth rate limiters key on the caller's IP (`_client_key` in `backend/routers/session.py`). Behind a reverse proxy every request shares the proxy's own peer IP, so this reads the `CF-Connecting-IP` header (set, and any client-supplied value overwritten, by Cloudflare for traffic it proxies — see the hosted deployment's architecture) and falls back to the raw peer IP when that header is absent (local dev, Docker Compose, or direct access with no Cloudflare in front). This assumes Cloudflare is the only path in; it's spoofable by anyone reaching the app directly via its Fly hostname, which isn't yet blocked (tracked separately).
