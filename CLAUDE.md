...
- **`integration` and `main` are kept in step in *both* directions, and neither is ever pushed to directly.** Dependabot targets `integration` (`.github/dependabot.yml`) so a week of routine bumps costs one deploy instead of one per PR. `.github/workflows/integration-promote.yml` carries that batch into `main`; `.github/workflows/integration-sync.yml` carries `main` back into `integration` and catches up any PR the move left behind. The return leg is not housekeeping — it is what keeps the forward leg working. The promotion squash-merges, which advances `main` and leaves `integration` exactly where it was, and `integration-branch-protection` sets `strict_required_status_checks_policy`, so the moment `integration` trails `main` every open Dependabot PR reports `mergeable_state=behind` and can no longer merge. That is not hypothetical: on 2026-08-25 six of them were stranded that way, the oldest for a fortnight, while the bumps that did land had been vetted against a base weeks stale and were then promoted into a `main` they had never been tested against. Both branch rulesets require a pull request, allow only squash merges, and name no bypass actors — so anything that needs to move either branch opens a PR, including the workflows above.
