...
- **A recommendation judgment is paid for once per *record*, and never expires.** Two things keep it that way, and both look redundant until you remove one. (1) `stock_item_judgments` has no foreign key on `item_key`, so `replace_stock_items`' delete-and-reinsert cannot touch a verdict, and price is not in `compute_item_key`'s hash, so a repriced item is the same item. A judgment is a permanent snapshot: nothing re-judges on a collection change, and the only thing that deletes one is the user pressing clear. (2) The *identity* it is billed on is the record, not the listing — `(_artist_sort_sql(artist), record_key)`, where `record_key` (`title_key.py`) folds away the pressing-variant words a store fences off in a bracket or trailing segment. Coarser than `title_key` there, and deliberately *finer* in two places, because a pressing key may be forgiving where a record key may not: it keeps word order and repeats (`title_key` compares a sorted set, which makes "Love Hate" and "Hate Love" one key), and it spares the noise words a record can be named with (`_NAMEABLE_NOISE`), leaving those to the same fence rule, so "Record One" and "Album One" stay two records — dropping them everywhere, as `title_key` still does, makes both of them the single key "one". Both directions cost an extra judgment at worst; the merges they prevent cost a verdict on the wrong album. `get_unjudged_stock_items` returns one representative per unjudged record — carrying the title its key was folded from (`COALESCE(NULLIF(listing_title, ''), title)`), not the catalog target a release crawler was searching for, which on that path can name a different record entirely — and `db.propagate_stock_judgments` copies the verdict onto that record's other in-stock, unowned listings, so every reader still matches listing-for-listing while the model is asked once. Keying on `item_key` alone re-bills a re-slugged URL, a second shop, and every colour variant — the leak that motivated this. **Verdicts are matched to records through `stock_item_identities`, never through `stock_items`**: identities are only ever upserted, stock rows are deleted wholesale every sync, and a join through the snapshot silently fails for the re-listing case above — the single most important one. **Which record a verdict was about is the verdict's own `record_key`, not the identity's**: a verdict is keyed by `item_key`, one `item_key` can carry live rows folding to two records (see below), and the identity holds only one of the two — so reading the record off it attributes a verdict about one record to the other, suppressing that other record from the billable set and then propagating the verdict, reason and all, onto its real listings. A NULL there means "not recorded" (written before the column, or imported, since an import carries no attribution — and an import *clears* the column on a row it updates, so a verdict it replaced cannot lend its record to the one replacing it) and falls back to the identity *only* where the `item_key` is unambiguous — no live row of it says anything else, **an unkeyed row included**: a row the sweep has not folded yet has not agreed, and reading its silence as agreement is the same false merge in slower motion. Deliberately not backfilled: where the fallback is safe the two agree anyway, and where they do not the identity is the guess the column exists to stop trusting. Because that match runs one table's `record_key` against the other's, the identity must hold the key of **at least one** of its live stock rows — each write aligns the pair for the one observation it writes, which is not a promise that the two always match: both live writers compute the key once and write it to both, and `db.backfill_stock_keys` re-folds the stock rows and then takes each identity's key *from* one of its live stock rows rather than re-deriving it. **One `item_key` may legitimately carry several live rows whose keys differ**, because it hashes artist, title and URL while `record_key` folds the *listing* title, so two crawlers finding one record at one URL fold apart — the schema permits the duplicate rows deliberately. The identity can equal only one of them, and that is a residual state rather than corruption: an identity already holding the key of *any* of its live rows is left alone, and only one matching none is repaired. Do not "fix" that into picking a winner. Nor narrow the `item_key` floor by the verdict's `record_key`: two rows under one key are *the same product page* named two ways by two crawlers, so the floor gives that URL its one verdict, and narrowing it would leave the second row both unbillable and blank. No column says which write committed last (`last_seen` is `CURRENT_TIMESTAMP`, the transaction start), so any ordering disagrees with the live writers for some interleaving, and the sweep and the writers then take the identity in turns — which lets listings of one record inherit a verdict written about the other. What the collision costs instead is bounded at the billing end: the billable set and its count are deduplicated by `item_key`, since a verdict is stored per `item_key` and, without that step, two record groups sharing one would buy a second model call and nothing else — which is the call the deduplication prevents. Two things about that sweep are load-bearing, and each was got wrong once by reaching for the cheap version. **It recomputes rather than asking which keys are missing**, because a missing key is not the dangerous way a stored one goes wrong. The deploy is rolling, so an old binary goes on writing `stock_items` after a new one has added the column, and its `ON CONFLICT DO UPDATE` names neither key it does not know about — *preserving* them while `listing_title` moves to a new name. Nothing is NULL to mark it, the identity beside it keeps the same stale value so the disagreement test sees agreement, and the listing silently joins a record it is not and takes that record's verdict. That is a false **merge**, the one direction the whole design errs away from. The rule enforced instead has no such gap: the stored pair must equal the fold of the row's own current artist/title/listing_title, which is exactly what both live writers put there. **And its writes are conditional on the source fields still reading as the `SELECT` found them** — not on the key columns, because the writer this exists for is precisely the one that never touches those. The folds happen in Python while the crawl worker pool holds no part of the stock-sync lock, so under `READ COMMITTED` a writer can commit between read and write, and putting both passes in one transaction would buy atomicity rather than the isolation that needs — which is also why they are not in one. An unconditional write would put a stale fold over a newer title in *both* tables, leaving them equal and both wrong. Neither pass is cheap any more (~850 ms per sweep on a 9,000-row catalog, against ~23 ms for the NULL-only version it replaced), so both callers run it off the event loop. Ahead of both, a **`BEFORE UPDATE` trigger on each table clears a fold key whose source fields moved without it** (`clear_fold_keys_left_behind`, `clear_identity_fold_key_left_behind`). The sweep repairs a stale key but cannot repair it *fast*, and the gap matters: between one sweep returning and the judgment queries that follow, a stale key is non-NULL, so the billable set compares it, groups the listing under a record it is not, and can hand it that record's verdict — which then survives every later sweep, because the `item_key` floor reads it as judged for good. The write that creates the hazard is the write that clears it, and NULL is the one value every reader already handles. Each key is judged separately (an old binary knows `title_key` and writes it correctly), the sweep never trips it (it writes keys and leaves sources alone), and a title changing to one that folds the same way costs one re-fold — the cheap side of a test that cannot itself fold. **It also commits between its two passes**, so it never holds a `stock_items` row lock while reaching for a `stock_item_identities` one: that pairing deadlocks against `upsert_stock_item_from_release`, which takes them the other way round. Matching a writer's order is not available — `replace_stock_items` deletes its crawler's stock rows *before* upserting identities, so the two live writers already disagree with each other — and holding neither lock across the other conflicts with neither. The cost is a window where a stock row is keyed and its identity is not, which the next sweep repairs and which errs toward billing a sibling twice rather than billing a verdict to the wrong record. Propagation is scoped to exactly the rows that would otherwise be billed (in stock, unjudged, not owned), which is what keeps the CSV export meaning "verdicts you paid for" rather than filling with records the user already owns. `inherited` — the listings that took a verdict rather than being paid for — is both broadcast on `stock_judgment_complete`/`stock_judgment_stopped` and recorded on the `stock_judgment_runs` row, because that event reaches subscribers of the Machine running the job and a client polling from the other one would otherwise be told a run that spent nothing checked nothing. `start_judgment_only` also refuses while `stock_sync_running`, because judging a half-replaced catalog buys verdicts on items about to be deleted; the mirror guard on `start_stock_sync` is deliberately absent (judgment is per-user, the sync is global). See [`docs/specifications/shaping/2026-09-16-record-level-judgment-design.md`](docs/specifications/shaping/2026-09-16-record-level-judgment-design.md).
...
- Counts a single sentence defines by enumerating, where the number *is* the
  fact — "`colCount` is 8 under the Collection filter and 7 otherwise" describes actual columns
...
**Never enable auto-merge on a PR you open, and disable it if you find it already enabled** (`gh pr merge <PR> --disable-auto`) before ending your turn. `main`'s branch-protection ruleset now requires 1 approving review before merge — raised from 0 after PR #134 merged without one, closing the race at the platform level so no PR can merge unattended regardless of what triggers merge. But GitHub's own Copilot code review (`copilot_code_review.review_on_push`) still runs asynchronously and is not itself a required reviewer, so the required-approval rule stops an *unattended* merge, it doesn't guarantee whoever approves has actually seen Copilot's comments first — a human can still approve before that review has posted. This was a real, observed failure before the ruleset change: on PR #134 (`required_approving_review_count: 0` at the time), CI going green and auto-merge together let the PR merge in 2 seconds, with Copilot's review posting 20 seconds *after* the merge, too late to matter. Do not close that gap by letting Copilot — or any bot — hold the approval. (That design doc argues this from Copilot's review being Comment-only, which was true when it was written and is not any more: as of 2026-09-16 Copilot submits `state: "APPROVED"` on a pull request's final head. GitHub documents that its approvals still do not count toward the requirement; this repo has *not* verified that either way, because the owner's bypass means no merge here settles it. The instruction is unchanged regardless — it rests on what an automatic approval would do, not on Copilot being unable to give one.) Engineering a bot approval would make the requirement self-satisfying, since the `copilot_code_review` rule reviews a pull request when it opens (`review_on_push: true` adds one per push after that), putting the first review on a head that is rarely the one that merges, and `dismiss_stale_reviews_on_push: false` means no later push clears it. See [`docs/specifications/shaping/2026-08-29-copilot-pr-approval-design.md`](docs/specifications/shaping/2026-08-29-copilot-pr-approval-design.md), which records the reasoning and the live gaps in the ruleset (`required_review_thread_resolution`, `dismiss_stale_reviews_on_push` and `require_last_push_approval` are all `false`, so an approval survives every later push and does not require Copilot's own inline comments to be resolved first).

Scope: that rule is about PRs *you* open. It is not a blanket ban on `--auto` in the repo, and in particular it is not a licence to strip `--auto` out of `integration-promote.yml`, `integration-sync.yml`, or `dependabot-auto-merge.yml` — batching a week of Dependabot bumps into one deploy is the whole point of those workflows, and nothing they land escapes review. The promotion PR into `main` still waits on `main`'s required approving review; the sync PR only targets `integration`, which requires no approval and carries no Copilot review rule, and only ever replays content already reviewed on its way into `main`; `dependabot-auto-merge.yml` only ever lands a bump on `integration`, and that batch still meets the promotion PR's review before any of it can deploy.

A single check right after CI passes has the same race shifted one step later: Copilot's review can still land after that check finds nothing, and #134's own review + comments were posted together, after the merge. Checking merely "does *a* review from `copilot-pull-request-reviewer[bot]` exist" is also insufficient once you've pushed more than once: `/pulls/{number}/reviews` returns the PR's full history, so that check is satisfied by a stale review left over from an earlier push while a newer push sits unreviewed. Before considering a PR-opening task finished: capture the head SHA right after your final push (`git rev-parse HEAD`), then **poll** `gh api repos/{owner}/{repo}/pulls/{number}/reviews --paginate` every ~20-30s until a review from `copilot-pull-request-reviewer[bot]` whose `commit_id` matches that exact SHA appears (the reviews endpoint is paginated too — on a long-lived PR with many review rounds, the matching review can land past the first page while an unpaginated poll loops to timeout without ever seeing it), capped at ~5-6 minutes (Copilot's #134 review took ~3.5 minutes end to end) — if it times out, say so explicitly rather than silently treating "no review yet" as "no feedback." A matching review ends the *poll*, but is not necessarily the last word on that head: Copilot can review the same commit again with no push in between, so re-check for a newer review before calling the PR finished (see the closeout rule below). Once that review has landed, fetch inline comments with `gh api repos/{owner}/{repo}/pulls/{number}/comments --paginate` (an unpaginated call can silently truncate on a PR with many comments). If a PR merges before this check can complete (auto-merge enabled by something outside your control), treat any legitimate finding the same as if caught pre-merge: fix it and open a small follow-up PR immediately, don't let it slide because the original PR is already closed. For each inline comment you address after the fact, reply on that comment thread (`gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies`) naming the fixing commit, then resolve the thread — but only for comments that actually have a `comment_id` (a "suppressed"/non-inline comment in a review body has none; it is covered by the rule below instead).

**Close out every Copilot finding explicitly, so that on an ordinary PR no open Copilot thread means you're done.** An open thread from `copilot-pull-request-reviewer[bot]` means you are still working on it; a resolved one means you dealt with it and the reply above it says how. Two scope limits, and the second is not the first restated. The signal is over *Copilot's* threads rather than GitHub's unresolved-conversation badge, which doesn't distinguish authors — the owner's own threads stay open by design (below), so a nonzero badge is not by itself evidence you're unfinished. But filtering by author does not rescue every case: on the bot-opened promotion PR you never resolve at all (below), and those are *Copilot's* threads, so an open one there means you finished and left the resolve to a human. Thread state cannot carry the signal on that one PR — the running ledger comment does instead, kept there whether or not the round had suppressed findings. Two things make that signal worth reading, and without them it is worse than nothing. **Reply before you resolve** — naming the fixing commit, or, where you're declining, what you checked and why the finding doesn't hold — because a silently resolved thread is indistinguishable from an addressed one and reads as addressed. And **resolve only after the fix is pushed**, so the sha you name is on the branch. Resolved means "dealt with", not "agreed"; the owner can unresolve a thread to push back, which is the cheap way for them to reject your answer. Resolve outdated threads too (`is_outdated` means the code moved under the comment, not that anyone answered it), and leave the owner's own review threads for the owner to resolve — their resolution and yours would claim different things. **Nothing is closed out on an unreviewed head:** both halves of the signal describe the commit Copilot last reviewed, so your own fix-and-push silently makes them stale — resolved threads stay resolved, the ledger still reads complete, and if the new head's review is clean or body-only, nothing reopens to correct it. You are done only once the current head has a matching review (the poll above) *and* its findings are closed out. Have the ledger name the review and head it covers, so it goes stale visibly rather than needing a flag you have to remember to flip. **Its absence has to mean something too:** a review can raise only body-only findings and open no thread, so an unprocessed PR shows no open thread *and* no ledger — silence a reader takes for "nothing to do". So write the ledger when you process the *first* review on a PR, whether or not that round had suppressed findings. The reading, in order: no review on the current head → not readable yet; any Copilot review the ledger doesn't name → not processed; ledger naming the newest Copilot review with no open Copilot thread → done, as of that review. **Name the review, not just the head** — Copilot can review the same commit twice with no push between, and the second can reverse the first: PR #337 has review `5161031901` (`🟢 Approved`) and review `5179396360` (`🟡 Changes recommended`, four findings) both on `b4607e5`, 38 hours apart. So "a review exists on this head" is satisfied by the stale one; naming the review subsumes the head check rather than adding to it, and "done" means as of the newest review seen, never that no further review will arrive.

The reply and the resolve take **different ids**, and only the reply is REST. Replying takes the comment's numeric id from its `#discussion_r<id>` anchor (`gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies`, or `mcp__github__add_reply_to_pull_request_comment`). Resolving takes the *thread's* GraphQL node id (`PRRT_…`): with `gh`, the `resolveReviewThread` mutation through `gh api graphql`; without it — the remote/web sessions have no `gh` — `mcp__github__pull_request_read` with `method=get_review_comments` returns every thread's `id`, `is_resolved` and `is_outdated` next to its comments, and `mcp__github__resolve_review_thread` takes that `id`. The review poll and the review *bodies* need an MCP path too, not just the threads — the rule above states both in `gh` terms while this one notes the remote/web sessions have no `gh`, which leaves them unable to do the very poll that gates finishing. Use `mcp__github__pull_request_read`: `method=get_reviews` for the review list and bodies (where suppressed findings live), `method=get_review_comments` for threads. **Paginate both — and they paginate differently, which is a live trap:** `get_reviews` is page-based (`page`/`perPage`) and returns the PR's whole history oldest-first, so the newest review sits on the *last* page and a first-page read shows you the oldest rounds; `get_review_comments` is cursor-based, returning `pageInfo.hasNextPage` and `endCursor` to pass back as `after`. Reading one page of either is how a session concludes "nothing new" while a round sits unread on the next. Don't filter threads by an exact author match without checking the spelling the call you used actually returns — Copilot's login has three forms: `get_reviews` gives `copilot-pull-request-reviewer[bot]`, `get_review_comments` gives the same bot bare as `copilot-pull-request-reviewer`, and GraphQL gives its inline comments as `Copilot` ([`docs/specifications/shaping/2026-08-29-claude-on-pull-requests-design.md`](docs/specifications/shaping/2026-08-29-claude-on-pull-requests-design.md) records the two-identity problem and the silent miss it already caused once). Match loosely; an exact-match filter that hits nothing looks exactly like Copilot having nothing to say. Sweep the threads as part of the review poll above, not before it: `review_on_push: true` means the round that most needs closing out is the one landing on your final head. Don't stop at a `🟢 Approved` review either — it is not terminal, and PRs here routinely collect further `🟡 Changes recommended` rounds after one.

**Threads are only half of what Copilot says, so resolving them all is not the same as answering it.** Findings below its inline-confidence bar land in the review *body* instead, under a `### Suppressed comments (N)` heading as `**path:line**` entries (sometimes under a further `**Previously missed (N)**` sub-heading) — no thread, no `comment_id`, no resolve state, nothing to reply to. They are not the minor half: on recent PRs they have outnumbered the inline findings, and have carried crash and correctness bugs. So read every review body, and account for each suppressed finding — fixed, or declined and why — in **one running PR comment you edit in place** each round, not a fresh comment per round. A PR with no unresolved threads and unread suppressed findings is the exact failure this rule exists to prevent: it reads as handled. See [`docs/specifications/shaping/2026-09-16-copilot-review-thread-resolution-design.md`](docs/specifications/shaping/2026-09-16-copilot-review-thread-resolution-design.md), which also records why this must not be turned into a merge gate. Concretely: **on the bot-opened promotion PR, never resolve a thread** — fix what the finding names and reply saying so, but leave every resolve there to a human, and let the ledger carry that PR's completion. Unconditional on purpose: the rule used to apply only while `required_review_thread_resolution` was enabled, but that setting lives on GitHub and can be turned on without any file here changing, so an agent trusting this file's dated `false` would clear the production merge gate itself. A condition you must check is worse than an action you never take.
