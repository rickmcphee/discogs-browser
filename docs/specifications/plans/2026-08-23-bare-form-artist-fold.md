...
Expected: FAIL. First test: `get_distinct_artists` returns `["Beatles", "Beatles, The"]` (two entries) instead of one. Second test: `artists` includes a stray `"Beatles"` entry instead of the merged `["Beatles, The"]`.
