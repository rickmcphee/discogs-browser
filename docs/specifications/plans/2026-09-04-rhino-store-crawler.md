...
- **A guard must cover the availability field itself, and must read the outcome rather than the field.** `variants` vanishing, or `available` vanishing from every variant, makes every product yield nothing while the other tallies stay non-zero — a successful empty walk that deletes the snapshot. Count *unreadable* qualifying products, not readable ones: an "at least one readable" test is satisfied by a single genuinely sold-out product on behalf of a catalog that has gone unreadable behind it. Gate the raise on the walk having yielded nothing, so an isolated unreadable product stays an ordinary skipped row. "Readable" means a literal boolean, matching the filter's own `is True` — anything the filter would skip must not count as readable, or it vouches for an emptiness it causes.
