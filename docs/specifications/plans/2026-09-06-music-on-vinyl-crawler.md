...
- No new shared module — reuse `shopify_catalog.iter_products()`, `strip_vendor_prefix()` and `resolve_cover_image()` unchanged.
...
**Interfaces:**
- Consumes: `shopify_catalog.iter_products(base_url, collection_slug)`, `strip_vendor_prefix(title, vendor)`, `resolve_cover_image(product, variant)` — all exist unchanged in `backend/shopify_catalog.py`.
