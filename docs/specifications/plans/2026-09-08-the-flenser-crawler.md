...
- **Tallies are nested** (type → parse → format → variant gate → identity/readable), so a non-zero count means "some product would have yielded a row if it were in stock", and are taken **before** the availability filter, so a sold-out store is empty legitimately.
