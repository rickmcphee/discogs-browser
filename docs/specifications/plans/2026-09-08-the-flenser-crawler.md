...
- [x] **Step 13: Address Copilot's fifth review round** — a payload field that is not a string reached `.split()` and aborted the whole source; reported on the variant title, and three sibling fields (`product_type`, product title, handle) had the identical defect. One `_text()` helper now reads every string field, so an unreadable one is *absent* and lands in a guard that names it. Plus a handle validated collapsed but interpolated raw into the URL, and the broadest drift guard ordered ahead of the specific ones, which made `variant-source drift` and `pressing-name drift` unreachable for a single-product catalog.
- [x] **Step 14: Address Copilot's sixth review round** — the descriptor quote rule reversed to refusing the glyph outright, after a third instance of the same ambiguity (`Artist "The " 12" LP` is structurally identical to `Artist "Album" 12"`); `parsed == 0` masking `identity-source drift` when every product loses its title, fixed by ordering every guard most-specific-first; and a `variants` field retyped to a truthy scalar raising `TypeError` out of `list()`, fixed with one shared reading of that field. A surviving mutation also exposed the mark fold in the new quote check as a no-op, and it was removed rather than tested around.
