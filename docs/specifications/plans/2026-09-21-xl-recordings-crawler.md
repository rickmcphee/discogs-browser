...
- **`vendor` beats a tag that is a broken fragment, but not one that is a leading credit.** `Tyler, The Creator` arrives comma-split and `vendor` is simply right. `Gil Scott-Heron & Jamie xx` is the other case: the tag names the first of two collaborators, which is the only form the library can match, because `parse_release()` keeps `artists[0]` alone and `_library_release_match_sql()` compares the artist for equality. So a vendor is reduced to a sole tag when that tag is a leading prefix followed by a credit separator — space-delimited, never a comma, since a comma there is part of the name.
