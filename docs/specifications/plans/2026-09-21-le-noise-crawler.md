...
- [x] **Step 7: Run the wider crawler test selection for regressions** (`pytest tests/ -k crawler` with the three test env vars set — the plugin loader imports every module in `backend/crawlers/`, so a syntax error in the new file breaks unrelated tests). When this step first ran, two order-dependent failures (`test_queue_router`, `test_crawler_crud`) reproduced identically with the new files removed, and each passed in isolation. **Re-run on the final head, only `test_queue_router::test_broad_rows_fan_out_to_every_eligible_crawler` still fails**; `test_crawler_crud` now passes inside the selection. The selection's membership changed underneath this record as `main` merges added crawler test files — which is exactly what an order-dependent failure is sensitive to — so the line is corrected to the final run rather than left pointing a reader at a failure that is no longer there. Noted rather than diagnosed: neither failure is this branch's.
...
- [x] **Step 10: Work the review rounds on the PR.** Every finding was verified against the code before acting, and every one was real; none was declined. The bullets below are *representative*, not the full record — the complete round-by-round log, including the findings Copilot suppressed into review bodies with no thread, is the review ledger on the PR itself. A count here would be stale by the next round, and was:
  ...
  - *Round 11* — two suppressed findings, both arithmetic in prose rather than in code. An amendment this branch had written into the Waterloo design scaled a sampled in-stock rate by the page-count ratio, when that rate came from the alphabetical head of a *different* collection and the page ceiling truncates alphabetically — so the scaled figures were withdrawn rather than refreshed. And this design costed the rejected sitemap at three orders of magnitude when the measured ratio is about 400×. The lesson: an amendment a branch writes is a change like any other and earns the same scrutiny, and "proportionally lower" is a claim about the data, not a restatement of the arithmetic.
