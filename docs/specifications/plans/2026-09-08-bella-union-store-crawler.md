...
- **The format gate reads the descriptor only**, never the product title: bundle/garment first, then record word, then other medium, then a bare garment size, defaulting to admit. Its rejecting vocabulary spells cassette `casse+tte`, because the store's live catalog contains `Casseette` on a product whose other variants are records.
...
- **A title-less product counts toward `identity_missing` before the classification tests**, since the parse reads the title and such a product can never be classified. **A product read and deliberately skipped — the store's merch, or one with no record among its formats — does not count toward `unclassifiable`**; that test runs first, so a sold-out CD-only product cannot raise drift. Neither half of it reads the title, so it exempts a non-record whose title did not parse either; requiring a readable album there made the branch depend on the one field it must not. **`identity_missing` and `unreadable_stock` are otherwise nested inside the record classification**, so a non-zero count means "some product would have yielded a row if it were in stock".
