...
- **A title-less product counts toward `identity_missing` before the classification tests**, since the parse reads the title and such a product can never be classified. **A product read and deliberately skipped — the store's merch, or one with no record among its formats — does not count toward `unclassifiable`**; that test runs first, so a sold-out CD-only product cannot raise drift. **`identity_missing` and `unreadable_stock` are otherwise nested inside the record classification**, so a non-zero count means "some product would have yielded a row if it were in stock".
