...
- **`artist` is `vendor` unless the vendor is the label** (`Matador Records`, `MatadorRecordsProd`), in which case it is the first non-housekeeping tag — the catalog keeps a release's primary artist alone and the Track tab's library match is exact, so credits are never joined. A label-vendored product with no credit left is skipped, never credited to the label. A tag never overrides a real vendor.
