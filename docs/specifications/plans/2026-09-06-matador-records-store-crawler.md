...
- **`artist` is `vendor` unless the vendor is the label** (`Matador Records`, `MatadorRecordsProd`), in which case it is the non-housekeeping tags. A label-vendored product with no credit left is skipped, never credited to the label. A tag never overrides a real vendor.
