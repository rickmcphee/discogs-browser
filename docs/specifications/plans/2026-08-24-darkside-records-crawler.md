...
- **No pre-order handling, because there are no current pre-orders here.** The `preorder_bt` (874) and `pre-order vinyl` (271) tags are stale residue on already-released shelf stock: every tagged product's release date is in the past, the two tags never co-occur, and every product in this collection is tagged `instore-available`. So neither half of the sibling pattern applies — bypassing the availability gate is inert (the store marks these `available: true`), and *labelling* would mark over a thousand released, in-stock records as pre-orders.
...
- [x] **The no-id fallback reinstated it a third time.** With placeholder titles *and* no variant `id`, `_compose_title` still returned the bare album. Since `replace_stock_items` deletes this crawler's rows before inserting, that would have replaced a good snapshot with colliding ones. Fixed by raising: `_sync_stock` catches it and `continue`s past `replace_stock_items`, so the previous snapshot survives. Every path through `_compose_title` now either yields a distinct descriptor or raises.
