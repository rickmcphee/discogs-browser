...
**Amendment (2026-08-29):** the bullet above treats a "Failed to fetch"-style message as evidence that a request failed in transit. That was not reliably true when this was written. Starlette's `ServerErrorMiddleware` builds an unhandled exception's 500 outside every `add_middleware()` layer, `CORSMiddleware` included, so on this deployment — SPA and API on different hosts — a backend that was up and *erroring* also produced a bare `TypeError: Failed to fetch`, because the browser blocked the response before the app could read its status. A backend-down page keyed off that message alone would therefore have covered for genuine server errors too. `unhandled_exception_headers` now re-applies the CORS headers as well as the security ones, so an error response is readable and only a real transport failure rejects. The detection this document specifies is unaffected — it keys off `checkHealth()`, not off any one request's rejection message — but the distinction the bullet draws between "the whole backend is down" and "one request failing" only actually holds now.
