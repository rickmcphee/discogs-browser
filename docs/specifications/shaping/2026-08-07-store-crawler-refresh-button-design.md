...
- In the SSE handler: `stock_sync_started` sets `setStockSyncTarget(event.crawler_id ?? 'all')` (in addition to the existing `setSyncing(true)`/`setSyncStatus(...)`); `stock_sync_complete` and `stock_sync_aborted` each add `setStockSyncTarget(null)`. **(2026-09-17, PR #368: the spinner is no longer raised in that handler. `setSyncing(true)` there became a share of a set of owners, `beginSyncing('stock')`, and then left the handler altogether — `spinnerOwnerOf` takes the share from *any* `stock_sync_*` event at the top of the SSE dispatch, since a stream reconnecting after the start event has left the replay buffer never sees it. The endings give the share back through `endSyncing`, which is what keeps the non-terminal `source`-carrying error below from being mistaken for one. `stockSyncTarget` is unaffected. See [`2026-08-22-live-recommended-filter-design.md`](../../superpowers/specs/2026-08-22-live-recommended-filter-design.md).)**
