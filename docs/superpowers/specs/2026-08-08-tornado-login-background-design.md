...
- Follow-up tweak: `HOOPS` size progression runs small-near-the-bottom to large-near-the-top (matching how a real tornado narrows at the ground and widens toward the cloud), and the smallest hoops near the bottom overlap each other for visual density; total funnel height (`cy` span) is unchanged.

## Non-goals

- No app rename (title, storage keys, headings) — separate future task. Update: the title and login heading were renamed to "Track Tempest" in #74; storage keys are still untouched.
