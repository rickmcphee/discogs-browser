...
**Amendment (2026-07-31, crawl-queue-refactor Task 21):** this entire feature is removed. The multi-tenant refactor ([`2026-07-26-multi-tenant-architecture-design.md`](2026-07-26-multi-tenant-architecture-design.md), [`2026-07-27-crawl-queue-refactor-design.md`](2026-07-27-crawl-queue-refactor-design.md)) drops Plex matching as an explicit non-goal (`users.plex_base_url`/`plex_token`/`plex_match_threshold` are unused columns, no outbound calls). Task 21 finished the removal on the frontend: `RecordBrowser.tsx`'s `plex_url` hyperlinks and `plexAvailable`/`no_plex` filter dropdown, `App.tsx`'s `plex_match_*` SSE handling, and `Settings.tsx`'s `plex_base_url`/`plex_token` fields are all gone; `CrawlEvent`'s status union no longer lists `plex_match_started/progress/complete/error`. Everything below describes removed, unshipped behavior.
