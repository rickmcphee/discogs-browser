...
**Amendment (2026-07-31, crawl-queue-refactor Task 21):** the single-owner feature described below is removed and unshipped, but it is *deferred, not abandoned* — the earlier wording of this note said the multi-tenant refactor "drops Plex matching as an explicit non-goal," which misattributed the decision and overstated it. [`2026-07-26-multi-tenant-architecture-design.md`](2026-07-26-multi-tenant-architecture-design.md) still **plans** per-user Plex, as decomposition item 4, gated behind SSRF hardening; only [`2026-07-27-crawl-queue-refactor-design.md`](2026-07-27-crawl-queue-refactor-design.md) declares it a non-goal, and only of *that plan*. What is actually gone: `users.plex_base_url`/`plex_token`/`plex_match_threshold` and `library_items.plex_url`/`plex_matched_at` survive as columns no code reads or writes, `backend/plex.py` survives imported only by `backend/tests/test_plex.py`, and no outbound Plex call happens anywhere. Task 21 removed the frontend: `RecordBrowser.tsx`'s `plex_url` hyperlinks and `plexAvailable`/`no_plex` filter dropdown, `App.tsx`'s `plex_match_*` SSE handling, and `Settings.tsx`'s `plex_base_url`/`plex_token` fields are all gone; `CrawlEvent`'s status union no longer lists `plex_match_started/progress/complete/error`. Everything below describes removed, unshipped behavior.

**Amendment (2026-08-01, branch `plex-reachability-ssrf`):** decomposition item 4 has now been implemented — per-user, with SSRF hardening — on branch `plex-reachability-ssrf` (stacked on `crawl-queue-refactor`, not yet merged). The "Task 21 removed the frontend... are all gone" sentence above is no longer accurate as a description of the current tip of that branch: `RecordBrowser.tsx`'s title-hyperlink treatment is rebuilt (per-user `plex_url`, same two-tier link/plain-text shape as below, but not the `plexAvailable`/`no_plex` filter dropdown, which stays out of scope), `App.tsx`'s `plex_match_*` SSE handling is rebuilt, and a Plex settings section is rebuilt in `Account.tsx` (not `Settings.tsx` — per-user settings moved there during the OAuth/multi-tenant work). See [`2026-08-01-plex-reachability-ssrf-design.md`](2026-08-01-plex-reachability-ssrf-design.md) for the current, authoritative design — this document's amendments above remain accurate as history of the single-owner→removed transition; only the "are all gone" clause is superseded.
