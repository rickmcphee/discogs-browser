...
Fixing it by widening the empty-state selectors was rejected for the reason the rest of this crawler is built on: the live DOM still cannot be verified from a sandbox Cloudflare challenges on every request, so `.marketplace_empty` and the `has-text` strings beside it are guesses of the same kind as the card-row selectors already abandoned, and a guessed empty state fails in the destructive direction — it clears a stored price. What settles the question without guessing is a second read of the same release with the `ships_from` parameter dropped and nothing else changed (`Crawler.unfiltered_url()`), scored by the same `_read_when_ready()` the first read used. Recognising that page proves the selectors still parse this site, which leaves the filter as the only thing that could have emptied the first one, and the crawler returns `[]` — a confirmed "nothing ships from the USA", which is an answer rather than a fault. Failing to recognise it means the selectors really have gone stale, and the raise stands with its message now recording that the unfiltered read failed too. The check therefore distinguishes the two hypotheses in production, which no amount of reasoning from a challenged sandbox could: a genuine restyle still breaks both reads and still raises, while an absence of domestic sellers no longer does.

Four properties hold it in place. It costs a page load only on the path that was already about to raise, so the common case is untouched. An unknown count (the stats API declining to answer) reaches the same check, since a verified-parsing unfiltered page settles the question whether or not Discogs will say how many copies exist.

The other two came out of Copilot's review of the branch, and both guard the same destructive direction. **The empty answer is gated on the first response having been a clean one** — a 4xx/5xx, or a `cf-mitigated` response whose title was not on the challenge list, rendered no listings for a reason that has nothing to do with the filter, so letting a *later* request's success speak for it would turn a block page into "no USA sellers" and clear the stored price. When the first response was not clean the second read is not made at all: nothing it could say would be about the filter. (`test_the_http_status_is_named_when_markup_is_not_recognised` had anticipated exactly this shape — "the edge served a block page with a title that isn't on the challenge list" — while passing for an unrelated reason.) And **a stall or an interstitial on the second read is re-raised in the shape the first read gives it**, rather than folded into the markup complaint, because the pool keys its recovery on the exception type: a Playwright `TimeoutError` is what makes `CrawlManager._process_claimed_rows` discard the crawler's browser context, so a second navigation that stalls has to escape as one or the dead socket pool is inherited by every job after it; and `BotDetectedError` is what makes `_paced_search` reset the context and retry, which a challenge on this read deserves as much as a challenge on the first. Laundering either into a `RuntimeError` would spend the recovery and blame the selectors for an edge that was merely blocking us.
