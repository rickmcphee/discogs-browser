...
Three properties hold it in place. It costs a page load only on the path that was already about to raise, so the common case is untouched. "Could not tell" is folded into "could not read" — a stall or an interstitial on the second read leaves the complaint standing rather than manufacturing an empty result, because raising costs a crawl while a wrong `[]` erases a price the crawler had already found. And an unknown count (the stats API declining to answer) now reaches the same check, since a verified-parsing unfiltered page settles the question whether or not Discogs will say how many copies exist.
