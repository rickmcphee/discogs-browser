...
**Amendment (2026-09-07, branch `claude/discogs-crawler-selectors-dmt4de`):** the "markup not recognised" raise the 2026-08-29 amendment introduced began firing on releases whose markup was fine. The reported case was `Crimson Glory - Chasing The Hydra (r37054242)` — HTTP 200, `cf-mitigated=None`, a settled page title naming the release, and the stats API reporting copies for sale — a page, in other words, that arrived intact and was not challenged.

The fault is a mismatch between the two things that decision compares. `GET /marketplace/stats/{release_id}` counts copies **worldwide** and takes no `ships_from` parameter, while the page `search_url()` builds is narrowed to United States sellers. The crawler read a non-zero worldwide count as proof that an empty page must be broken markup, when the filter alone explains it: a release pressed for Europe, with every copy still in Europe, renders an honest "nothing matches" and is then reported as a selector failure. This was not a near miss but a structural one — the `num_for_sale == 0` branch is the only route to an empty result, and it is reachable only for a release with no copies anywhere on earth, so *every* release with foreign sellers and no domestic one was guaranteed to raise no matter how well the selectors worked. The damage compounds: those raises count toward the per-site breaker (`empty_result_is_expected` exempts empty results, not exceptions), so a run of import-only records cools Discogs off for thirty minutes — the precise outcome that attribute was added to prevent, arriving by the other door.

Fixing it by widening the empty-state selectors was rejected for the reason the rest of this crawler is built on: the live DOM still cannot be verified from a sandbox Cloudflare challenges on every request, so `.marketplace_empty` and the `has-text` strings beside it are guesses of the same kind as the card-row selectors already abandoned, and a guessed empty state fails in the destructive direction — it clears a stored price. What settles the question without guessing is re-reading, and two separate things have to hold before an empty page may be blamed on the filter. **The selectors must still work**, which a read of the same release with the `ships_from` parameter dropped and nothing else changed (`Crawler.unfiltered_url()`) establishes by parsing. **And the filtered page must render no listings a second time**, because `_read_when_ready()` reports an exhausted deadline and unrecognised markup identically — so without that confirmation a clean 200 whose listings were merely late looks exactly like "no USA sellers", and the unfiltered read, made moments later against a site that has since sped up, would vouch for it. Both reads go through `_verify_read()`, which scores them with the same `_read_when_ready()` the first read used. Only then does the crawler return `[]` — a confirmed "nothing ships from the USA", which is an answer rather than a fault. If the confirming read finds the listings after all, they are returned, which beats both the empty result and the raise.

What must reproduce on that confirming read is the **absence of listings**, not a recognised empty state, and the distinction is load-bearing rather than pedantic: the premise of this entire path is that Discogs's empty state is markup this crawler cannot name, so demanding a recognised one would mean the case the fix exists for could never reach the empty result and every import-only release would go on raising. `_verify_read()` therefore reports `answered` (the response was clean) separately from `recognised` (the DOM rendered something known), and the two callers take the strength of evidence each needs.

Failing either condition means the raise stands, its message recording which. The check therefore distinguishes the hypotheses in production, which no amount of reasoning from a challenged sandbox could: a genuine restyle breaks both reads and still raises, while an absence of domestic sellers no longer does. Note the standing cost — an import-only release now spends three page loads per pass where it used to spend one and raise. Capturing the real empty-state markup would let the *first* read recognise it and retire this path altogether; that remains the proper fix, and this is what correctness looks like until someone can see the live DOM.

It costs those loads only on the path that was already about to raise, so the common case is untouched. An unknown count (the stats API declining to answer) reaches the same check, since a verified-parsing unfiltered page settles the question whether or not Discogs will say how many copies exist.

The rest came out of two rounds of Copilot review on the branch, and every one guards the same destructive direction — an empty result clearing a stored price. The confirming re-read described above is one of them. **The empty answer is gated on the first response having been a clean one** — a 4xx/5xx, or a `cf-mitigated` response whose title was not on the challenge list, rendered no listings for a reason that has nothing to do with the filter, so letting a *later* request's success speak for it would turn a block page into "no USA sellers" and clear the stored price. When the first response was not clean the second read is not made at all: nothing it could say would be about the filter. (`test_the_http_status_is_named_when_markup_is_not_recognised` had anticipated exactly this shape — "the edge served a block page with a title that isn't on the challenge list" — while passing for an unrelated reason.) And **a stall or an interstitial on the second read is re-raised in the shape the first read gives it**, rather than folded into the markup complaint, because the pool keys its recovery on the exception type: a Playwright `TimeoutError` is what makes `CrawlManager._process_claimed_rows` discard the crawler's browser context, so a second navigation that stalls has to escape as one or the dead socket pool is inherited by every job after it; and `BotDetectedError` is what makes `_paced_search` reset the context and retry, which a challenge on this read deserves as much as a challenge on the first. Laundering either into a `RuntimeError` would spend the recovery and blame the selectors for an edge that was merely blocking us. **Every verification read is held to the first read's cleanliness standard**, for the reason that motivated the gate in the first place: a read whose own response was a block or an error cannot be evidence for the empty result, and an error body may well carry a container this crawler would parse.

**Amendment (2026-09-06, branch `claude/discogs-crawl-timeouts-f3p6vc`):** in production the crawler's navigation itself began failing wholesale — `Page.goto: Timeout 30000ms exceeded … waiting until "domcontentloaded"` on release after release — and that line could not say what had happened. `domcontentloaded` only fires once the whole document has arrived and its synchronous scripts have run, so a response that started but never finished (a slow origin, or Cloudflare stalling a client it has scored as a bot) burned the full window and reported exactly what a connection that was never answered reports. Navigation now resolves at `commit` (response headers received, document committed) with an explicit 30 s timeout; the settle-and-parse polls the 2026-08-29 amendment introduced already decide readiness, so `domcontentloaded` was buying nothing. Consequences: a navigation timeout now means one thing — no response headers inside the window — and is logged as such before the Playwright `TimeoutError` is re-raised unchanged (not wrapped, and not `BotDetectedError`: a stall gives no reason to expect an immediate retry to fare better, and `[]` would clear the stored price); an empty `<title>` is treated as unsettled by `_await_settled_title`, because at `commit` the parser may not have reached it and a challenge read at that instant would otherwise pass the bot check; and the HTTP status plus `cf-mitigated` header of the committed response are carried into the "bot interstitial did not clear" warning and the "markup not recognised" raise, so an operator can tell a restyle from a block page from the log alone. On the pool side, `CrawlManager._process_claimed_rows` now discards the worker's browser context for a crawler whose `search()` escaped with a Playwright `TimeoutError` (`_discard_context`), so the next request to that site opens on a fresh socket pool rather than inheriting a connection the far end silently dropped — see the 2026-09-06 amendment to [`2026-08-01-worker-pool-pacing-design.md`](2026-08-01-worker-pool-pacing-design.md). What this does **not** do is establish why Discogs stopped answering the Fly machines; it makes the next failure line say which of "no response at all", "challenge never cleared" or "page arrived but unreadable" it is, and stops one stalled connection from being reused.
...
Consequently the crawler declares `empty_result_is_expected = True`, which until now had been reserved for single-store crawlers rather than marketplaces. The breaker only has to infer breakage from emptiness while a crawler cannot separate "nothing here" from "I could not read this" — the exclusion is for crawlers that can. This one separates them itself and raises on the second, so the breaker already has its signal by that route and the remaining empties are confirmed answers. Counting them only cooled the site off over releases with no USA seller.

Two decisions above are revised rather than merely elaborated. "No fuzzy matching / scrape the first row" assumed `sort=price,asc` is honoured; since that parameter is still unconfirmed, the cheapest listing is now chosen from the parsed rows rather than taken from page order, so a silently ignored sort cannot make the first row masquerade as the cheapest. And row parsing now also reads `data-pricevalue`, the attribute Discogs's own currency-switching JS reads, alongside the legacy `td.item_price .price` node. Both are scoped to a container Discogs is actually known to use (`table.mpitems`, or the `#pjax_container table` this crawler shipped with), and every row shape carries a price node `_parse_row()` can read, so a selector cannot match a row it has no way to parse. Supporting a *restyled* layout was tried and abandoned: guessing at card rows under `[class*='marketplace']` cannot be scoped, because that container is the whole app in any plausible layout, so a recommendation card nested under it parses as a listing and — the cheapest winning — becomes the release's price. With the live DOM unverifiable, a guessed selector that half-works is worse than none, and an unrecognised restyle raises. For the same reason there is deliberately no bare `[data-pricevalue]` sweep at any point — not as a row selector, where `amazon.py` already learned that scraping every price-shaped element picks up carousel prices and, since the cheapest wins, a recommendation would become `matches[0]`; and not in the readiness wait, where a stray price element anywhere on the page would satisfy it before the listings render, reinstating the very race being fixed. A restyle that moves listings out of every known container raises, which is the intended outcome rather than a gap to be closed with a broader net. The wait covers the empty states as well as the rows, so a genuine no-listings page is recognised as soon as it renders instead of burning the full timeout first — and those empty states are themselves scoped to the container and required to be `:visible`, since an unrelated or stale-hidden "no items for sale" string would otherwise end the wait early and be read as a confirmed miss, clearing the stored price while the listings were still rendering.

Still unverified, and now failing loudly instead of silently: the selectors and the `ships_from` parameter could not be confirmed from the sandbox this was written in, which Cloudflare challenges on every request. *(2026-09-07: still true, and the 2026-09-07 amendment above stops the unverified `ships_from` parameter's ordinary effect — an empty page for a release with no domestic seller — from being misreported as a selector failure.)* Fixtures under `backend/tests/fixtures/crawlers/discogs_marketplace/` encode the markup the crawler claims to support, not markup captured from the live site.
