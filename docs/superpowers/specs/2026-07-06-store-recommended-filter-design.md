...
**Amendment 6 (2026-07-31):** the `crawl-queue-refactor` multi-tenant migration made two claims below inaccurate. `judgment_running` is no longer a bare property — it's now `judgment_running(user_id)`, backed by a per-user `_judgment_tasks: dict[int, asyncio.Task]` in `CrawlManager` (judgment always ran against one calling user's own taste listing and Anthropic key, so a single global flag incorrectly blocked one user's judgment run on another's, the same bug class fixed for collection sync's `sync_running` earlier in that migration). The "Mutual exclusion" bullet under [Decoupled judgment-only refresh](#3-decoupled-judgment-only-refresh) claiming `start_stock_sync` and `start_judgment_only` each refuse if either is running was already inaccurate before this migration — neither checks the other today, only `POST /api/stock/judge/clear` refuses while either is running (that part of the bullet is still correct) — and remains that way after the per-user change: `start_stock_sync` (still process-global, one shared `stock_items` catalog) and `start_judgment_only` (now per-user) run independently of each other.

**Amendment 7 (2026-07-31, branch `crawl-queue-refactor`):** a correction to Amendment 6 above, plus three drifts it left unmentioned. **Correction:** Amendment 6's claim that the "Mutual exclusion" bullet "was already inaccurate before this migration — neither checks the other today" is wrong. Both cross-guards existed on the parent branch (`start_stock_sync` and `start_judgment_only` each checked `stock_sync_running or judgment_running`) and were removed *by* this migration, not before it — lines under [Decoupled judgment-only refresh](#3-decoupled-judgment-only-refresh) described real shipped behavior until then. Today `start_stock_sync` checks only `stock_sync_running` and `start_judgment_only` only `judgment_running(user_id)`. Dropping `judgment_running` from `start_stock_sync` follows from the per-user change (one user's judgment run must not block the global stock refresh), but dropping `stock_sync_running` from `start_judgment_only` was not deliberate: a user starting judgment mid-stock-refresh judges a partially-replaced catalog and spends Anthropic tokens on items about to be deleted and reinserted. Because `item_key` is a content hash rather than a row id, the cost is wasted work rather than corruption. ~~Still open.~~ **Closed 2026-09-16** (branch `claude/wizardly-goldberg-mxvey2`): the guard is back, and lives in `start_judgment_only` rather than the router so no call site can start a run around it. `start_stock_sync` deliberately keeps *only* `stock_sync_running` — that half of the removal was correct. `POST /api/stock/judge/start` now also returns `stock_sync_running` so the Refresh button can say which of the two guards turned it away instead of rendering nothing. See [`2026-09-16-record-level-judgment-design.md`](../../specifications/shaping/2026-09-16-record-level-judgment-design.md). **Further drifts:** (1) the judgment phase no longer runs inside `_sync_stock()` — it is manual-only, per user, via `POST /api/stock/judge/start` → `start_judgment_only(user_id)`, so the "Runs once per stock sync" premise below is retired, and a missing key now broadcasts `stock_judgment_error` rather than being a silent in-sync no-op. (2) `stock_item_judgments` is a per-user RLS-scoped Postgres table with composite PK `(user_id, item_key)` — see [`2026-07-27-crawl-queue-refactor-design.md`](2026-07-27-crawl-queue-refactor-design.md)'s Data model — so the SQLite `CREATE TABLE` and every unscoped SQL snippet below is stale; `has_any_stock_judgment`, `clear_stock_judgments`, `get_recommended_stock_items`, `get_taste_listing` and `get_unjudged_stock_items` all take `user_id`, and taste is built from that user's `library_items`, not a flat `releases` table. (3) `anthropic_api_key` and `recommendation_item_limit` are no longer global `config.json` fields on `GET`/`POST /api/settings` (now admin-only) — they are per-user `users` columns read/written via `GET`/`POST /api/user-settings` and surfaced in `Account.tsx`'s "Recommendations" section, not `Settings.tsx`; `_run_judgment_phase` reads both off the calling user's own row, `App.tsx` derives `hasAnthropicKey` from `getUserSettings()`, and the "Refresh Recommendations" button is no longer disabled on key presence (the requirement is stated in the section blurb instead). This supersedes Amendment 5's pointer to `Settings.tsx` for current placement. Not being rewritten here.

**Amendment 19 (2026-09-16, branch `claude/wizardly-goldberg-mxvey2`):** judgment is billed per *record* now, not per listing. The "Judgments persist across resyncs" goal below still holds exactly as written — it was never the part that leaked — but the identity it persists *on* was too narrow: `item_key` hashes the URL, so a shop re-slugging a product, a second shop stocking the same record, and a different pressing of it were three separate paid judgments for one taste question. `get_unjudged_stock_items` therefore returns one representative per unjudged record rather than one row per unjudged listing, and a new `db.propagate_stock_judgments` copies a record's verdict onto its other in-stock, unowned, unjudged listings so every reader below still matches listing for listing. Three consequences for text below: `count_unjudged_stock_items` is no longer `SELECT COUNT(DISTINCT s.item_key)` — it counts one representative per unjudged record, then deduplicates those by `item_key`, since two records can share the single row that stores their verdict, so it always equals the set the model is sent; `stock_judgment_complete` carries an `inherited` count beside `judged`; and `stock_items`/`stock_item_identities` gain a `record_key` column (title_key.py's record fold — coarser than `title_key` on fenced pressing variants, finer on word order, repeats and nameable noise words). `stock_item_judgments` keeps its `(user_id, item_key)` primary key and its CSV export format, and gains one nullable column, `record_key`: which record the verdict was reached about, as opposed to which listing it is filed under. It has to be stored rather than looked up, because one `item_key` can carry live stock rows folding to two records while `stock_item_identities` holds only one key — so reading the record off the identity attributes a verdict about one record to the other. NULL means "not recorded" (a verdict predating the column, or an imported one, which carries no attribution) and falls back to the identity only where the `item_key` is unambiguous; the column is deliberately not backfilled. `judge_batch`'s return shape gains the same field, carried through from the batch entry rather than re-derived, which supersedes Amendment 17's "its return shape is unchanged" for this one field. Two corrections to Amendment 18, which landed on `main` while this was in review: `POST /api/stock/judge/start` answers `{"started", "running", "run", "stock_sync_running"}` — the fourth field because `start_judgment_only` refuses while a stock sync is replacing the very catalog it would be judging, and that refusal deliberately runs *before* the claim, so it writes no run row for the other three to be read from; and the button's description, already noted there as having drifted twice, drifts again, since a finished or stopped run now also reports how many listings inherited a verdict rather than being paid for. See [`2026-09-16-record-level-judgment-design.md`](../../specifications/shaping/2026-09-16-record-level-judgment-design.md).
...
**Amendment 13 (2026-08-22, branch `store-filter-recommendations-refresh`):** the "`Recommended` gating" section under Amendment 2 (§4, below) is superseded. `App.tsx` no longer tracks a `judgmentRunning` boolean and `recommendedAvailable` no longer gates on "no judgment run currently in progress" — it is now just `hasAnthropicKey && hasJudgedItems`, so `Recommended` stays selectable throughout a refresh once at least one judgment exists. Separately, `hasJudgedItems` now also flips true progressively off `stock_judgment_progress` (first non-zero batch), not only off `stock_judgment_complete`, so a user's very first-ever refresh unlocks `Recommended` mid-run. And the concern in §4's opening paragraph — that `StockBrowser` "has no SSE awareness and won't reflect newly-judged items until something re-triggers its fetch" — is resolved rather than avoided: both `stock_judgment_progress` and `stock_judgment_complete` now bump `stockSyncGeneration`, so the item list and artist sidebar live-repaint each batch. See [`2026-08-22-live-recommended-filter-design.md`](2026-08-22-live-recommended-filter-design.md) for the full design; not being rewritten here.

**Amendment 12 (2026-08-16, branch `store-saved-items`):** "now just All/Recommended" above is superseded — the Store dropdown gained `Saved`, for a per-user "save for later" bookmark unrelated to `Recommended`. `Recommended` itself is unaffected. See [`2026-08-16-store-saved-items-design.md`](../../specifications/shaping/2026-08-16-store-saved-items-design.md).

**Amendment 15 (2026-08-26, branch `claude/store-overlapped-artist-filter-i3cp7i`):** the Store dropdown gained `Overlapped`, and Amendment 8's account of `Overlapping` being removed needs reading carefully next to it. What Amendment 8 removed was the *release-level* filter (artist **and** title), and it is still gone — it lives on as the Track tab's Collection option. `Overlapped` matches at *artist* level: any in-stock record by an artist the user collects, owned or not. `Recommended` is again unaffected, though the two are now near neighbours — `Recommended` is the paid LLM judgment and excludes records already owned, `Overlapped` is a free structural heuristic and does not. See [`2026-08-26-store-overlapped-artist-filter-design.md`](../../specifications/shaping/2026-08-26-store-overlapped-artist-filter-design.md).
...
**Amendment 10 (2026-08-10, branch `cleorecs-vinyl-crawler`):** this amendment originally existed to restate a catalog-source count that had gone stale; the counts it corrected have since been removed from the body text below, and per `CLAUDE.md` no replacement count is recorded. What remains worth keeping is the substantive re-check it carried: the prefix invariant the body text below relies on was re-confirmed against the sources that had shipped since it was written — `piratespressrecords.py` stores either the bare clean title or `f"{title} (Pre-Order)"`; `sgrecordshop.py` stores the bare clean title only (its own `@`-split strips all variant/color text before storage, so there's never a suffix to begin with); `cleorecs.py` stores either the bare `album` or `f"{album} — {variant_title}"`. All three keep the clean title as a literal, space-terminated-or-exact prefix — none breaks the invariant.

**Amendment 11 (2026-08-10, branch `claude/cleorecs-vinyl-crawler-265d08`):** a correction to Amendment 10 above. Amendment 10 reasoned only about `catalog`-type crawlers, but the claims it revisits describe the sources `_sync_stock` crawls into `stock_items` — and `_sync_stock` (`backend/crawl_manager.py`) enumerates `get_enabled_crawlers("catalog") + get_enabled_crawlers("catalog_browser")`, not `"catalog"` alone, so the `catalog_browser` crawlers `amoeba.py` and `angryyoungandpoor.py` are in scope for every one of those claims too. The prefix-invariant re-verification Amendment 10 ran therefore missed them; checked here for completeness: `amoeba.py` stores the row's title verbatim with no suffix appended (trivially a prefix of itself), and `angryyoungandpoor.py` stores either the bare title or `f"{remainder} (Used)"` (space-terminated prefix). Neither breaks the invariant. (This amendment originally also reconciled two conflicting source counts; per `CLAUDE.md` those counts are no longer recorded here.)

**Amendment 14 (2026-08-23, branch `claude/spv-store-crawler-2mdf0z`):** many catalog sources have shipped since Amendment 11's enumerative pass, the most recent being Waterloo Records (#169), merged while this branch was open; each is documented under [`docs/specifications/shaping/`](../../specifications/shaping/) rather than amended in here. This amendment records no total, and the totals Amendment 11 carried are gone — per `CLAUDE.md`, a count of things that change is not written down in this repo. Read every source count still in the body text below as "all catalog sources," not a literal figure. What actually matters here is the prefix invariant the body text depends on, and it still holds for the source added on this branch. `spv.py` stores the bare parsed album title; `f"{album_title} — {variant_title}"` for a product with more than one meaningful variant; and either of those with `" (Pre-Order)"` appended — four shapes, not the two an earlier draft of this amendment listed, because the variant qualifier was added later on the branch and this enumeration was not revisited until review on PR #165 caught it. The qualifier is inserted before the pre-order suffix, never after, so in every shape the clean album title remains an exact or space-terminated prefix. `test_variant_qualifier_precedes_the_preorder_suffix` asserts that ordering directly, against this invariant.

**Amendment 16 (2026-08-29, issue #224):** the reason-text instruction quoted under [Reason-text style + externalized prompt](#2-reason-text-style--externalized-prompt) is superseded — `backend/recommendations_prompt.md` no longer requires an impersonal, factual-description-only reason. Feedback was that the "no second-person references" rule (introduced by the first amendment above) produced text that read as a dry justification for why an item qualified rather than an actual recommendation. The reason is now written as a one-sentence recommendation addressed directly to the collector as "you," still required to name the specific band, label, or genre that explains the fit rather than relying on vague enthusiasm alone. A reason is now written only for a recommended item: `reason` is null when `recommended` is false, because `get_stock_items` returns the stored reason on every row regardless of that flag and the mobile list renders any non-null reason, so recommendation wording on a rejected item would surface as a recommendation the judgment did not make. `stock_item_judgments.reason` is already nullable and the write path already passes a missing key through as NULL, so this is a prompt change only.

**Amendment 18 (2026-09-16, branch `claude/lucid-maxwell-69049q`):** a recommendation run can now be stopped, and the mechanics of starting one moved into Postgres to make that possible. Supersedes the start/refusal claims under [Decoupled judgment-only refresh](#3-decoupled-judgment-only-refresh) and the endpoint shapes with them. **What changed.** One run per user is a `stock_judgment_runs` row — status, counters, a `stop_requested` flag, a heartbeat and a claim token — claimed by `start_judgment_only` before it creates its task. That row, not `CrawlManager._judgment_tasks`, is what refuses a second start: the deployment runs two Machines behind one hostname, so the per-process check covered only the half of the requests that landed on the Machine holding the task, and on the other half it started a second run judging the same items on the same user's Anthropic key. The row also expires (`JUDGMENT_RUN_STALE_MINUTES`), so a Machine that died mid-run no longer refuses that user's Refresh for good. `judgment_running(user_id)` still exists and is still per-process, but its only caller is `_events_to_replay`, which is asking about this process's own replay buffer. **The stop is cooperative.** `_run_judgment_phase` reads `stop_requested` at every batch boundary — in the same statement that records progress and proves the claim — and closes the run as `stopped` when it is set, keeping every judgment already committed. It is not a `task.cancel()`: the Anthropic call runs on a worker thread through `asyncio.to_thread`, which a cancelled await does not interrupt, and cancelling mid-batch would discard a response the user has already paid for, leaving those items to be judged and billed again. The cost is up to one batch of latency, which the button reports as `Stopping…` rather than claiming an instant stop. A new `stock_judgment_stopped` event carries the count a stopped run reached; it is tagged with the owning `user_id` like the rest of the family. **Endpoints.** `POST /api/stock/judge/start` answers `{"started", "running", "run"}`, with `running` and `run` read from the row rather than from this process. `GET /api/stock/judge/status` answers `{"any_judged", "run"}` — so the section below claiming a bare `{"any_judged": bool}` is superseded. New `POST /api/stock/judge/stop` answers `{"stopping", "run"}`, and answers `stopping: false` rather than an error when there is no live run, because asking a run that has just finished to stop is not a fault. `POST /api/stock/judge/clear` and the CSV import guard now read the row instead of `judgment_running` — under `db.lock_stock_judgment_run`, a per-user `pg_advisory_xact_lock` that claiming a run and a run's batch checkpoint take as well, in the same transaction as their own write, since an unlocked read is a check the answer outlives. Advisory rather than a row lock because the row need not exist: before a user's first run a `FOR UPDATE` is silently no lock at all. The lock orders those writes against each other but cannot order a batch against a clear that finished while the batch was still inside its Anthropic call, so a batch whose claim is gone is dropped rather than written — the guards mean what they say, at the cost of at most one batch, and only for a run that went stale with no replacement; they are racing the run's writes to `stock_item_judgments`, and that run is on whichever Machine served its start. One trap worth carrying forward: `user_scope` sets `app.user_id` transaction-locally, so a read placed *after* a `conn.commit()` in the same block evaluates a tenant policy against an empty string and raises — and no ordinary test here can catch it, because they connect as the superuser and bypass RLS entirely. `test_stock_router.py`'s `rls_enforced` fixture, which repoints the app pool at `app_user`, is what closes that gap; reach for it whenever correctness depends on the tenant policies actually running. **UI.** The `Refresh` row in `Account.tsx`'s Recommendations section is one button with three faces — `Refresh`, `Stop`, `Stopping…` — and App follows the run by polling `GET /api/stock/judge/status` while one is believed to be in flight, because the `stock_judgment_*` events reach only the Machine running it. The button's description quoted under [User-facing wording](#3-user-facing-wording-judgment--recommendation) has drifted twice over: its `"Requires an Anthropic API key above."` sentence was dropped before this branch, and it now carries a second sentence saying what Stop does. See [`2026-09-16-stop-recommendation-run-design.md`](../../specifications/shaping/2026-09-16-stop-recommendation-run-design.md).

**Amendment 17 (2026-09-07, branch `claude/recommendations-cost-optimization-ai8ev0`):** the request and response shapes in step 5 of [Judgment pipeline](#judgment-pipeline) are superseded, and two of the testing bullets with them. Items no longer travel to the model as `artist - title - item_key` triples, and the model no longer answers with `item_key`. Each item carries a 1-based ordinal `n` within its batch, the response is `[{"n": ..., "recommended": ..., "reason": ...}]`, and `judge_batch` maps `n` back to the batch's own `item_key` before returning — its return shape is unchanged, so `_run_judgment_phase`'s handling of the result, `upsert_stock_judgments` and everything downstream are untouched. `_run_judgment_phase` itself does change in one respect, covered below: it now passes the calling user's username into `judge_batch` so the usage counters can be attributed. The reason is cost: `item_key` is a SHA-256 hexdigest, hex tokenizes close to randomly, and the key was being sent in *and* echoed back for every item, output side bearing the higher rate — spend on a string the model can only copy. The ordinal also makes the ordering contract that step 5 always relied on checkable: an out-of-range or repeated `n` is dropped and logged, where an invented digest used to be written as a judgment against a row that need not exist (`stock_item_judgments` has no foreign key on `item_key`) while the real item stayed unjudged and was paid for again on the next run. Two smaller changes ride along: `response.usage`'s counters are logged per batch — nothing in the backend read them before, so the caching described under [Prompt caching](#2-prompt-caching) below could not be verified for any user — and a `max_tokens` stop is logged as itself rather than surfacing as the JSON parse error it used to become. That usage line is why `_run_judgment_phase` gained the one change noted above: judgment runs are per-user and several can be in flight at once, each on its own Anthropic key, with each batch dispatched through `asyncio.to_thread`, so an unlabelled counter interleaves into a stream nobody can attribute to a user or a key — and attributing them is the entire reason for logging them. **Correction to a testing bullet:** the `cache_control` assertion below is described as made "against the real request payload sent through the mocked HTTP layer"; it is not, and has not been since the `anthropic` SDK's transport dependency changed and `respx` (which patches `httpx` only) silently stopped intercepting anything. The test fakes the client object and asserts on `client.messages.create.call_args`. That predates this branch, which rewrote the file and so exposed it. The ~4096-token minimum-cacheable-prefix note under [Prompt caching](#2-prompt-caching) was re-checked and still holds. See [`2026-09-07-recommendation-judgment-token-cost-design.md`](../../specifications/shaping/2026-09-07-recommendation-judgment-token-cost-design.md).
...
- Hovering a recommended row's artist/title shows the one-line reason as a tooltip. **(Amended 2026-09-07, branch `claude/recommendation-info-icon-popup-6n4r87`:** clicking the row's info button shows it in a popover instead — a hover reached no touch device and announced nothing.**)**
...
The `' %'` (space-then-anything) branch exists because most of the store crawlers append variant info to the stored title (`"The Great Satan — Ghostly Black Vinyl"`), so a plain equality check would miss the majority of real matches. Every crawler's title-construction logic keeps the clean album title as a literal, space-terminated prefix (confirmed against every crawler's `title` field construction in [`2026-07-05-in-stock-crawler-design.md`](2026-07-05-in-stock-crawler-design.md), including every batch added after this spec was written — none of them break the prefix invariant), so this generalizes without per-source special-casing. The match is per (artist, title-prefix), not per artist alone — owning one release by an artist never suppresses a genuinely different release by the same artist.
...
- Clicking "Refresh Recommendations" with a configured API key judges currently-unjudged items and updates the status bar, without triggering a catalog re-crawl on any source.
...
**Amendment (2026-08-08, branch `button-color-consistency`):** `primaryButtonClass()` no longer applies to these buttons — the Settings/Profile action buttons (Settings' collection/price/stock "Refresh" and Account's "Refresh"/"Export"/"Clear"/"Link Now") were switched to `secondaryButtonClass()` (`bg-gray-700 hover:bg-gray-600 active:bg-gray-500`) to match the "Log out" button's existing gray scheme, standardizing all Settings/Profile action buttons on one color instead of two. `secondaryButtonClass()` itself gained the `active:bg-gray-500` state as part of this change (it previously had none), so the pressed-feedback behavior this section originally asked for carries over rather than being lost.
