...
**Amendment 14 (2026-08-23, branch `claude/spv-store-crawler-2mdf0z`):** the **36** Amendment 11 establishes is itself now stale — twelve further catalog sources have shipped since (eleven `catalog`-type, plus SideOneDummy Records as a third `catalog_browser`), each documented under [`docs/specifications/shaping/`](../../specifications/shaping/) rather than amended in here. Recounted with Amendment 11's own greps: **45** `catalog`-type plus **3** `catalog_browser`-type = **48**. Not correcting the body text's "31" figures in place, per this doc's convention of layering corrections; read every source count below as "all catalog sources," not a literal total. The prefix invariant the body text depends on is what actually matters here, and it still holds for the source added on this branch: `spv.py` stores either the bare parsed album title or `f"{album_title} (Pre-Order)"`, so the clean title is an exact or space-terminated prefix either way.
