...
**Amendment (2026-08-08, branch `button-color-consistency`):** `primaryButtonClass()` no longer applies to these buttons — the Settings/Profile action buttons (Settings' collection/price/stock "Refresh" and Account's "Refresh"/"Export"/"Clear"/"Link Now") were switched to `secondaryButtonClass()` (`bg-gray-700 hover:bg-gray-600`) to match the "Log out" button's existing gray scheme, standardizing all Settings/Profile action buttons on one color instead of two. `hover:`/`active:` press feedback is still present via `secondaryButtonClass()`'s own `hover:bg-gray-600` (no separate `active:` state on this helper, same as before this button used `primaryButtonClass()`'s `active:bg-gray-300` — a minor loss of an `active:` state, judged acceptable for visual consistency).
