...
**Amendment 10 (2026-08-10, branch `cleorecs-vinyl-crawler`):** the "31 sources"/"31 as of this writing"/"all 31 crawlers" counts below (in the ownership-exclusion bullet, the prefix-match rationale, and the two bullets under "3. Decoupled judgment-only refresh") are stale — 31 was the catalog-crawler total right after the last of the four planned genre-grouped batches ([`2026-07-05-in-stock-crawler-design.md`](2026-07-05-in-stock-crawler-design.md)'s `indie-alt-crawlers-1` amendment), and three more unplanned sources have shipped since: Pirates Press Records (32nd), The Sound Garden (33rd), and Cleopatra Records (34th, this branch) — see that spec's own amendment log for each. The real count as of this branch is **34**. Per this spec's own claim that the prefix invariant was "confirmed against all 31 crawlers'... including every batch added after this spec was written," the same check was re-run against these three: `piratespressrecords.py` stores either the bare clean title or `f"{title} (Pre-Order)"`; `sgrecordshop.py` stores the bare clean title only (its own `@`-split strips all variant/color text before storage, so there's never a suffix to begin with); `cleorecs.py` stores either the bare `album` or `f"{album} — {variant_title}"`. All three keep the clean title as a literal, space-terminated-or-exact prefix — none breaks the invariant. Not rewriting the body text below in place, per this doc's established convention of layering corrections as new numbered amendments rather than silently editing prior assertions.

**Amendment 11 (2026-08-10, branch `claude/cleorecs-vinyl-crawler-265d08`):** a correction to Amendment 10 above. **34 undercounts the actual source total.** All four claims Amendment 10 corrects describe the sources `_sync_stock` crawls into `stock_items` — and `_sync_stock` (`backend/crawl_manager.py`) enumerates `get_enabled_crawlers("catalog") + get_enabled_crawlers("catalog_browser")`, not `"catalog"` alone. Amendment 10's 31→32→33→34 progression tracks only the sequential `catalog`-type additions and never accounted for the two `catalog_browser` crawlers, `amoeba.py` and `angryyoungandpoor.py`, which also feed `stock_items` and are therefore in scope for every one of the four claims. Recounted directly against the tree (`grep -l 'crawler_type: str = "catalog"$' backend/crawlers/*.py | wc -l` → 34; `grep -l 'crawler_type: str = "catalog_browser"' backend/crawlers/*.py | wc -l` → 2): the true total is **36**. The prefix-invariant re-verification Amendment 10 ran also didn't cover the two `catalog_browser` crawlers; checked here for completeness: `amoeba.py` stores the row's title verbatim with no suffix appended (trivially a prefix of itself), and `angryyoungandpoor.py` stores either the bare title or `f"{remainder} (Used)"` (space-terminated prefix). Neither breaks the invariant.

**Amendment 14 (2026-08-23, branch `claude/spv-store-crawler-2mdf0z`):** the **36** Amendment 11 establishes is itself now stale — fourteen further catalog sources have shipped since (thirteen `catalog`-type, plus SideOneDummy Records as a third `catalog_browser`), the most recent being Real Gone Music (#164), merged while this branch was open, each documented under [`docs/specifications/shaping/`](../../specifications/shaping/) rather than amended in here. Recounted with Amendment 11's own greps, as of 2026-08-24: **47** `catalog`-type plus **3** `catalog_browser`-type = **50**. Like every count before it, that is a snapshot rather than a durable fact. Not correcting the body text's "31" figures in place, per this doc's convention of layering corrections; read every source count below as "all catalog sources," not a literal total. The prefix invariant the body text depends on is what actually matters here, and it still holds for the source added on this branch. `spv.py` stores four shapes, not the two an earlier draft of this amendment listed — the variant qualifier added later on the branch doubled them, and this enumeration was not revisited until review on PR #165 caught it: the bare parsed album title; `f"{album_title} — {variant_title}"` for a product with more than one meaningful variant; and either of those with `" (Pre-Order)"` appended. The qualifier is inserted before the pre-order suffix, never after, so in all four the clean album title remains an exact or space-terminated prefix. `test_variant_qualifier_precedes_the_preorder_suffix` asserts that ordering directly, against this invariant.
