...
- `async def search(self, release: dict, page) -> list[dict]` — navigates or queries the source, returns a list of `{url, price, shipping, currency, condition}` (empty when nothing matched) **(2026-09-05:** plus an optional `title` — the name the source itself gives the matched item, stored as `listing_title` and shown in place of the target's title on Store/Track rows; see [`2026-09-05-marketplace-listing-title-design.md`](../../specifications/shaping/2026-09-05-marketplace-listing-title-design.md)**)** **(2026-09-16:** and an optional `cover_image_url` — the picture the source showed for that same matched item, stored as `listing_image_url` and shown in place of the target's cover art on those rows, on identical terms; see [`2026-09-16-marketplace-listing-image-design.md`](../../specifications/shaping/2026-09-16-marketplace-listing-image-design.md)**)**. Every release crawler receives a live `playwright.Page`: `_drain_one_batch` creates a context for each eligible crawler and `_paced_search` hands that page to `search()` unconditionally. An API-based crawler simply ignores it and manages its own HTTP client — `ebay.py` does. **(2026-08-29:** `waterloorecords.py` was cited here too during its brief spell as a release crawler; it reverted to a catalog crawler and no longer implements `search()` at all. See the 2026-08-29 amendment to [`2026-08-24-waterloo-records-crawler-design.md`](../../specifications/shaping/2026-08-24-waterloo-records-crawler-design.md).**) **(2026-08-28:** read "API-based crawlers receive `None`" until now, which no dispatch path has ever done; corrected so a future API crawler implements what is actually passed.**) Raises `BotDetectedError` on bot interstitials (Playwright crawlers only). **(2026-08-09:** the return type read `-> dict` and a single `{...}` here until now; every crawler has always returned a list, as `CLAUDE.md` and every plugin show. Corrected in passing. Also: `[]` means "the site answered and has nothing" — an error must raise, or the consecutive-failure circuit breaker cannot see it. See item 9 of [`2026-08-01-worker-pool-pacing-design.md`](2026-08-01-worker-pool-pacing-design.md).)**
...
- `empty_result_is_expected: bool` — *optional*, added 2026-08-28, release crawlers. Declares that an empty `search()` result is a **confirmed miss**, not possible evidence of breakage — so it records no site-health signal instead of counting toward the consecutive-failure breaker. Only an explicit `True` opts out. Either of these earns that guarantee: a crawler for a single store, which legitimately will not stock most releases (as `waterloorecords` did during its spell as a release crawler); or a crawler that separates "the site has nothing" from "I could not read the page" itself and raises on the second, so the breaker already has its breakage signal by another route (`discogs_marketplace`). **(2026-08-29:** read "declares that this site legitimately will not have most releases", scoped to a single store rather than a near-universal marketplace, until now — one case stated as though it were the rule. `discogs_marketplace` meets the guarantee the other way, so the attribute is defined here by the guarantee itself, which is what the breaker actually relies on. See the 2026-08-29 amendment to [`2026-07-08-collection-price-crawlers-design.md`](2026-07-08-collection-price-crawlers-design.md).**) See item 8 of [`2026-08-01-worker-pool-pacing-design.md`](2026-08-01-worker-pool-pacing-design.md) and its 2026-08-28 amendment.
- `genre_summary: str` — *optional*, added 2026-08-12. One-sentence description of the kind of music the store sells; read by Settings to show as a hover tooltip on the store link so a user can decide whether to hide it. See [`2026-08-12-store-genre-summaries-design.md`](2026-08-12-store-genre-summaries-design.md). **(2026-08-28:** read "catalog/catalog_browser crawlers only" until now. Settings has always rendered the tooltip for any crawler carrying one, with no `crawler_type` gate, and the restriction only held because every named storefront happened to be a catalog crawler. Waterloo Records was at the time a `release` crawler that was still a named storefront with a real description (it has since reverted to `catalog`; the scoping rule stands either way), so the attribute is scoped by whether a crawler *is* a describable store, not by how it is crawled. See [`2026-08-24-waterloo-records-crawler-design.md`](../../specifications/shaping/2026-08-24-waterloo-records-crawler-design.md).**
...
**Amendment (2026-09-13, branch `claude/practical-cerf-kvjwbf`):** every event in step 3 is broadcast in-process, and the deployment runs more than one Fly Machine behind one hostname with no request affinity — so a browser whose SSE stream is served by the other Machine receives none of them for a refresh it just requested, and the collection table, which refetches only on those events, never shows what the sync fetched. The sync now also records itself in a `library_sync_runs` row (one per user: status, mode, scope, page/total_pages, counts, error, timings and a heartbeat), and `GET /collection/status` returns it as a `sync` object alongside `total`/`last_synced`; the client polls that while a run is live and drives the same state the SSE handlers do. Step 2's "already running" guard is no longer `_sync_tasks` alone: `start_sync` claims that row, so the `409` holds across Machines — previously the second Machine had no idea and would start a duplicate concurrent sync. A claim whose heartbeat has stopped for longer than `db.SYNC_RUN_STALE_MINUTES` is taken over, so a Machine that restarts mid-sync cannot hold a user's refresh hostage; such a run is reported as stale rather than running. The events in steps 3–5 are unchanged and remain the same-Machine fast path. See [`2026-09-13-collection-sync-run-visibility-design.md`](../../specifications/shaping/2026-09-13-collection-sync-run-visibility-design.md).

**Amendment (2026-09-16, branch `claude/sleepy-dijkstra-dlsrbx`):** step 3's `sync_error {error}` is unchanged in shape, but a sync that fails now always writes a line to the log as well, and for a Discogs HTTP error status the message names that status instead of reading "Discogs request failed". Two of the failures that end a sync before it fetches anything — no stored Discogs token, and the collection-fields call being refused — reported themselves only through this event and the `library_sync_runs` row, and logged nothing at all; against a Collection tab that then shows the same records as before, that is indistinguishable from a sync that found nothing new. A 401 or 403 is the stored OAuth token being refused rather than anything about the collection, so the message says so and names the remedy — signing in with Discogs again, which now actually replaces that token (see [`2026-07-26-discogs-oauth-auth-design.md`](2026-07-26-discogs-oauth-auth-design.md)). Discogs' own response body and the request URL stay out of the message, as before.
...
**Amendment (2026-09-13, branch `claude/friendly-lovelace-xn3c5i`):** step 4's 1.1-second delay is no longer the only thing keeping a sync inside the 60 req/min limit, and step 3's `sync_error {error}` no longer covers a single rate-limited page fetch. Every request in `discogs.py` now goes through `_get_with_retry()`, which waits out an HTTP 429 and retries — honouring `Retry-After` when the server sends one, otherwise backing off 5s, 20s, 60s, each wait capped at one rate-limit window and the retries capped at a fixed count. The pacing delay is unchanged and still does the ordinary work; the retry is what happens when it is not enough. That matters most for the page loops, which the delay does not pace at all: a 429 from `iter_collection_pages` / `iter_wantlist_pages` used to propagate to `_sync_collection_blocking`'s `except Exception` and end the whole sync, so every page after the failing one went unfetched and every release on those pages unsynced. Nothing about the *outcome* of a failure changes — a 429 that outlasts the retries raises the same `HTTPStatusError` to the same handler, and a failed barcode fetch is still logged without aborting the sync. See [`2026-09-13-discogs-api-429-retry-design.md`](../../specifications/shaping/2026-09-13-discogs-api-429-retry-design.md).
