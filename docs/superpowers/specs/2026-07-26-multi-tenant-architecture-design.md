...
**Amendment (2026-08-02):** "minted by hand" no longer describes the only path —
`POST /api/auth/invites` (admin-gated via `require_admin`, in `routers/session.py`)
mints a code through the app itself, returning `{code}`. This is still not the
