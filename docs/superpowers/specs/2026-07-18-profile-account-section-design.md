...
**Amendment (2026-09-16, branch `claude/lucid-maxwell-69049q`):** the `Refresh` row described in the amendment below is no longer a single-purpose button. It reads `Refresh` when nothing is running, `Stop` while a recommendation run is under way, and `Stopping…` — disabled — between a stop click and the run's actual end, since the run finishes the batch already paid for before it closes. It keeps `min-w-20` rather than the `w-20` its `Export`/`Import`/`Clear` neighbours carry, so it lines up with them at rest and grows for the longer label instead of clipping it. `Stop` posts to the new `POST /api/stock/judge/stop`; the state behind the three faces comes from `GET /api/stock/judge/status`, polled while a run is in flight, because the events that narrate one reach only the Machine running it. Still ungated, for the same reason the amendment below gives. See [`2026-09-16-stop-recommendation-run-design.md`](../../specifications/shaping/2026-09-16-stop-recommendation-run-design.md).
