...
- A nineteenth catalog source beyond the eighteen built here (the orchestration loop and `shopify_catalog.py` support it structurally, but no nineteenth crawler is being written now — three more genre-grouped batches of new sites are planned as separate work).
