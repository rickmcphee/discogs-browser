...
- **Artist casing (added later, branch `store-tab-overlapping-filter`):** `replace_stock_items` applies Python's `str.title()` to `item["artist"]` before insert — the single write path for `stock_items` (called from `CrawlManager._sync_stock`), so every catalog crawler gets normalized casing for free without per-crawler changes. Known tradeoff: `.title()` mangles some real band-name stylings (all-caps names like "NAILS" become "Nails"; it also mis-cases text after apostrophes) — accepted for consistent, predictable display over exact stylization fidelity. Applies at crawl time only; existing rows keep whatever casing they had until the next sync.
  ...
  **Amendment (2026-08-07, branch `fix-stock-artist-casing`):** revisited after the Pirates Press Records crawler's final code review flagged a worse case than the ones this bullet already accepted — that store has hyphenated, no-natural-single-capital names like "A-100s", and blanket `.title()` turns those into "A-100S" (capitalizing the letter right after the hyphen), which reads as a more obviously broken mangling than the ALL-CAPS case above, not just a stylization-fidelity tradeoff. `replace_stock_items` (via a new `normalize_artist_casing` helper in `db.py`) now only applies `.title()` when the input is entirely one case to begin with (`artist.isupper()` or `artist.islower()`); anything already mixed-case — including hyphenated names like "A-100s" and already-correct stylings like "The Beatles" — passes through untouched. The all-caps and post-apostrophe mis-casing tradeoffs above are otherwise unchanged and still apply to any all-caps or all-lowercase input (e.g. "NAILS" still becomes "Nails"); only the mixed-case path is new. Affects all catalog crawlers via the same shared write path.
