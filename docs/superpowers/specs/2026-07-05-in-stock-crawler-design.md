...
- Applies `crawl_delay_seconds` jitter before every request (category-page GET and every `/gsrp/` page GET) but does **not** read `consecutive_failure_limit` — there's no retry loop at all; any non-2xx from either endpoint propagates on the first occurrence. This is a real reduction in resilience relative to `iter_products()`'s retry-until-the-limit behavior for the 32 Shopify sites, not an equivalent alternative — accepted for now since no failure mode was observed to design around during live investigation; 80 sequential requests per crawl (14 categories × up to several pages each) makes a single transient failure costlier here than for any Shopify site, most of which complete in 1-3 requests total.
...
- `_sync_stock()` loads **all enabled catalog crawlers** (all thirty-three sites, plus any future one — the loop is data-driven off the `crawlers` table, not hard-coded), and for each: calls `crawl_catalog()`, replaces that crawler's `stock_items` rows, and broadcasts progress.
...
- "Refresh Stock Now" populates the Store tab with in-stock vinyl variants from all thirty-three sources, each priced and linked to its product page, with the correct source shown per row.
