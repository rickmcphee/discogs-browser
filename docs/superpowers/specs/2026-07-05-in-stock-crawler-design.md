...
- Applies `crawl_delay_seconds` jitter before every request (category-page GET and every `/gsrp/` page GET) but does **not** read `consecutive_failure_limit` — there's no retry loop at all; any non-2xx from either endpoint propagates on the first occurrence. This is a real reduction in resilience relative to `iter_products()`'s retry-until-the-limit behavior for the 33 Shopify sites, not an equivalent alternative — accepted for now since no failure mode was observed to design around during live investigation; 80 sequential requests per crawl (14 categories × up to several pages each) makes a single transient failure costlier here than for any Shopify site, most of which complete in 1-3 requests total.
...
- `_sync_stock()` loads **all enabled catalog crawlers of both kinds** — `get_enabled_crawlers("catalog") + get_enabled_crawlers("catalog_browser")`, so thirty-six sources today (the thirty-four `catalog` sites this spec enumerates plus Angry Young and Poor and Amoeba on the `catalog_browser` path), plus any future one — the loop is data-driven off the `crawlers` table, not hard-coded. For each: calls `crawl_catalog()`, replaces that crawler's `stock_items` rows, and broadcasts progress.
...
- A thirty-fifth catalog source beyond the thirty-four built here (the orchestration loop and the `catalog` crawler-plugin contract support it structurally, but no thirty-fifth crawler is being written now). **(2026-08-07 amendment:** this line originally said "thirty-second... this was the last of four planned genre-grouped batches" — Pirates Press Records was added afterward as an unplanned thirty-second source, so that claim is corrected in place rather than left stale.) **(2026-08-07 amendment, branch `store-crawler-sgrecordshop`:** this line then said "thirty-third catalog source beyond the thirty-two built here" and "the orchestration loop and `shopify_catalog.py` support it structurally" — The Sound Garden was added as an unplanned thirty-third source and doesn't use `shopify_catalog.py` at all, so both claims are corrected in place rather than left stale a second time.) **(2026-08-09 amendment, branch `cleorecs-vinyl-crawler`:** this line then said "thirty-fourth catalog source beyond the thirty-three built here" — Cleopatra Records was added afterward as an unplanned thirty-fourth source, so that claim is corrected in place rather than left stale a third time.)
...
- "Refresh Stock Now" populates the Store tab with in-stock vinyl variants from all thirty-six sources the stock sync enumerates (the thirty-four `catalog` sites this spec covers plus the two `catalog_browser` ones), each priced and linked to its product page, with the correct source shown per row.
