...
**Amendment (2026-09-06, branch `claude/store-cheapest-filter-x4tdwl`, second PR):** the filter dropdown is gone altogether: the Store and Track row-set filters and the Cheapest checkbox now sit behind one `Filter` popover trigger in the same anchored-dropdown / sheet shape as `Source` and `Stats`, immediately left of the list/tile toggle. The option sets and their behaviour are unchanged. See [`docs/specifications/shaping/2026-09-06-store-filter-popover-design.md`](../../specifications/shaping/2026-09-06-store-filter-popover-design.md); the 2026-09-05 amendment below describes the intermediate state.

**Amendment (2026-09-05, branch `claude/store-cheapest-filter-x4tdwl`):** the filter dropdown no longer sits directly "left of the list/tile toggle" as the Owned-item cross-reference bullet and the `StockBrowser.tsx` file note below say — on the Store tab a `Cheapest` checkbox now sits between them, keeping only the lowest-priced store's row for each record, grouped by a fuzzy fold of the title stored as `stock_items.title_key`. The `stock_items` listing under "Data model" is SQLite-era and already predates `item_key`, `release_id` and `listing_title`; it is left as history, as the branches that added those left it. See [`docs/specifications/shaping/2026-09-05-store-cheapest-filter-design.md`](../../specifications/shaping/2026-09-05-store-cheapest-filter-design.md).
...
**Amendment (2026-07-31, branch `crawl-queue-refactor`):** `shopify_catalog.py`'s own behavior is unchanged and everything the 2026-07-18 amendment describes still ships, but four cross-references in this spec are now stale. `crawl_releases()` — used above and below as the comparator for `iter_products()`'s delay/retry semantics — is deleted; more than that, the release crawl no longer applies `crawl_delay_seconds`/`consecutive_failure_limit` **at all** (the worker pool that replaced it implements neither), so `shopify_catalog.py` is now the only reader of those two settings rather than the variant behavior. `collection_schedule` (cited as the "existing `crawl_schedule`/`collection_schedule` pattern" that `stock_schedule` matches) and `scheduler.configure_sync` (cited as what `configure_stock` mirrors) are both deleted — collection sync is manual-trigger-only per user, so there is no collection schedule to match, and `scheduler.py` now has only `configure` (an all-users crawl sweep) and `configure_stock`. `GET`/`POST /settings` is admin-only now. `sync_running`/`start_sync()`, cited as the model `_sync_stock` was built on, are now per-user (`start_sync(user_id, mode)`); `_sync_stock` itself stays process-global, since `stock_items` is one shared catalog.

**Amendment (2026-08-01, branch `crawl-queue-refactor`):** one claim in the 2026-07-31 note above has already expired — `shopify_catalog.py` is **no longer the only reader** of `crawl_delay_seconds`/`consecutive_failure_limit`. The release crawl reads both again, per site rather than per run, via `CrawlManager._paced_search`/`_record_site_result` (see [`2026-08-01-worker-pool-pacing-design.md`](2026-08-01-worker-pool-pacing-design.md)). The two readers interpret the settings differently and that difference is now wider than the 2026-07-18 note describes: `iter_products()` retries a failed page until the limit is reached and then raises, aborting that catalog crawl, whereas the release path never retries beyond the single bot-detection retry and, on hitting the limit, pauses only that one site for 30 minutes without aborting anything. `crawl_releases()` remains deleted, so it stays unusable as the comparator this spec's text uses it as. `shopify_catalog.py`'s own behavior is still unchanged.
...
**Amendment (2026-08-05, branch `store-crawler-angryyoungandpoor`):** a new catalog source, Angry Young and Poor (`angryyoungandpoor.com`), is the first that doesn't fit this spec's `catalog`/`shopify_catalog.py` contract at all — it's PinnacleCart, not Shopify, and Cloudflare blocks any non-browser request outright (confirmed via `curl`, including on `robots.txt`). This requires a new `crawler_type="catalog_browser"` that gets a Playwright `Page` and can raise `BotDetectedError`, reusing the shared stealth Chromium instance and retry convention the release-crawl path already has — `_sync_stock` and `crawl_catalog()`'s zero-arg signature are otherwise unchanged for the existing `catalog`-type crawlers. Full design, including the accessory-filtering and category-dedup specifics: [`2026-08-05-angryyoungandpoor-store-crawler-design.md`](2026-08-05-angryyoungandpoor-store-crawler-design.md).

**Amendment (2026-08-07, branch `store-crawler-piratespressrecords`):** one more `catalog`-type (Shopify) source added — Pirates Press Records (`shop.piratespressrecords.com/collections/all`). Angry Young and Poor immediately above is `catalog_browser`-type, an architecturally distinct kind this spec doesn't enumerate in the sources-ship list below (**2026-08-07 amendment, branch `store-crawler-sgrecordshop`:** that list is no longer purely Shopify — see the amendment above). This store's nav has no single collection holding all of its vinyl (it's split across format-specific collections like `ppr-12-vinyl`/`ppr-7`/`ppr-10-vinyl`, plus a small `all-distro-titles` collection for vinyl the store distributes but doesn't press itself); an earlier draft of this crawler queried four of those collections and deduped the combined results by product `id`, but a wider check against the storewide `/collections/all` feed (1,317 products) found that approach missed 64 real vinyl releases — 496/560 covered — that `/collections/all` catches in fewer HTTP requests and with no dedup logic needed at all. The shipped crawler uses `/collections/all` instead, joining Rise Records/Saddle Creek/Kill Rock Stars/Triple B Records' existing `/collections/all`-plus-product_type-filter shape rather than introducing a new one. No change to `shopify_catalog.py`, the data model, the orchestration loop, the API, or the frontend. The "Out of scope" line below claiming no further `catalog`-type source would be built is now stale and corrected in place. See the new technical-grounding subsection below.

**Amendment (2026-08-07, branch `fix-hyphenated-artist-title-split`):** the same hyphenated-artist-name title-split collision Pirates Press Records' amendment just above found and fixed was also found live on Season of Mist — a plain `\s*-\s*` dash-split clipping a name like `"Cro-Mags"` to `"Cro"` — and fixed the same way, requiring whitespace on at least one side of the splitting hyphen. Three sibling crawlers carrying the byte-identical vulnerable regex (`fatherdaughterrecords.py`, `closedcasketactivities.py`, `triplebrecords.py`) were checked against live data and found clean — no code change needed for those three. See the updated Season of Mist technical-grounding subsection, and the negative-result bullets on the other three.

**Amendment (2026-08-07, branch `fix-hyphenated-artist-collision-followup`):** the same collision was checked live against the four remaining crawlers flagged as a follow-up by the amendment just above (`runforcoverrecords.py`, `twentybuckspin.py`, `polyvinylrecords.py`, `bigscarymonstersusa.py`). Run For Cover, 20 Buck Spin, and Polyvinyl Record Co. each had at least one real live title clipped by the old `\s*-\s*` split (`"Ultra-Lite"` → `"Ultra"`; `"PAN-AMERIKAN NATIVE FRONT"` → `"PAN"`; 8 titles on Polyvinyl including `"blink-182"` → `"blink"` and `"Sleater-Kinney"` → `"Sleater"`) and are fixed the same way, requiring whitespace on at least one side of the splitting hyphen. Big Scary Monsters USA's close variant (which also accepts en-dash) was checked and found clean — no code change there. No changes to `shopify_catalog.py`, the data model, the orchestration loop, the API, or the frontend. See the updated technical-grounding subsections below.

**Amendment (2026-08-09):** `_sync_stock` now applies the release path's per-site consecutive-failure breaker to catalog crawlers — `consecutive_failure_limit` failures in a row (429s excluded, they keep their own handling) put that source in a 30-minute cooldown, and a cooling-down source is skipped with an INFO line rather than crawled. Until now the stock path had no breaker at all beyond the 2-consecutive-429 run abort, so a site hard-blocking us (Amoeba Music's Cloudflare 403s) was re-attempted in full on every scheduled sync. The completion log also names failed and cooling-down sources, so "Stock sync complete: 0 items" no longer reads as a clean run at INFO. See items 13-14 of [`2026-08-01-worker-pool-pacing-design.md`](2026-08-01-worker-pool-pacing-design.md).

**Amendment (2026-08-10):** "loads **all enabled catalog crawlers** ... and for each: calls `crawl_catalog()`" under Backend orchestration below is now conditional on a second reading of that enabled set. `_sync_stock` still takes the snapshot once before the loop, but re-reads the live enabled catalog/`catalog_browser` ids per source inside it — in the same place it already re-checks `_cooling_down_crawler_ids()`, and for the same reason — and skips a source disabled since the run began, with an INFO line and a `disabled` entry in the completion log's notes tail. The skip happens before `stock_sync_source_started`, so a skipped source never announces itself. A store disabled while its own catalog crawl is already in flight finishes it and its items are written: the fetch has already happened and `replace_stock_items` with a truncated catalog would wipe good rows. See [`2026-08-09-stop-crawling-disabled-stores-design.md`](../../specifications/shaping/2026-08-09-stop-crawling-disabled-stores-design.md).

**Amendment (2026-08-07, branch `store-crawler-sgrecordshop`):** another `catalog`-type source, The Sound Garden (`sgrecordshop.com`), covering 14 of its "by genre" vinyl categories (its "used records" and "featured"/"new arrivals" categories are out of scope by explicit decision). Unlike every other `catalog`-type source this spec covers, it isn't Shopify — it runs FieldStack Omni — so it doesn't use `shopify_catalog.py` at all: a bespoke two-step flow (scrape a per-category `SearchId` token from category-page HTML, then paginate a `/gsrp/{page}` JSON AJAX endpoint with that token, reused across all of that category's pages rather than re-scraped per page) reaches listings rendered as an HTML fragment rather than clean JSON, parsed with regex rather than any HTML-parsing library (none exists in this codebase) or a live DOM query. No bot-blocking was found on this site, so it stays `crawler_type="catalog"` rather than needing Angry Young and Poor's `catalog_browser`/Playwright path — the first crawler to show that "catalog kind" no longer implies "Shopify via `shopify_catalog.py`," only "plain httpx, no browser." It reads `crawl_delay_seconds` for the same jitter-before-every-request convention `iter_products()` uses, but never reads `consecutive_failure_limit` — there is no retry loop at all, so a non-2xx from either the category-page GET or a `/gsrp/` GET propagates on the first occurrence, a real reduction in resilience relative to `iter_products()`'s retry-until-the-limit behavior, accepted for now since no failure mode was observed to design around during ~90 live requests made during this investigation. No changes to `shopify_catalog.py`, the data model, the orchestration loop, the API, or the frontend. The sources-ship list below is updated in place. Full design: [`2026-08-07-sgrecordshop-store-crawler-design.md`](2026-08-07-sgrecordshop-store-crawler-design.md).

**Amendment (2026-08-09, branch `cleorecs-vinyl-crawler`):** another `catalog`-type source, Cleopatra Records (`cleorecs.com`), covering its full `/collections/vinyl-1` collection — Shopify-based, so it does use `shopify_catalog.py`. `vendor` is the imprint (Cleopatra Records, Purple Pyramid, Deadline Music, and others) on every product, never the artist, so artist attribution is entirely title-parsing — joining Rev HQ, Deathwish Inc, Triple B Records, Big Scary Monsters USA, Polyvinyl Record Co., and Numero Group's existing vendor-is-unreliable-or-never-the-artist group (see the "Shared vs. per-site logic" bullet below). No pre-order override — this store's pre-orders already report `available: true`. Unlike every Shopify site above, filtering is a *non-vinyl-type exclusion set* (`{BND, BK, PS, PO, DVD, BR, TB, CD}`) plus a merch-title-keyword regex, not a positive vinyl-format regex — closer in shape to 20 Buck Spin's price/keyword filter than to any of the type-inclusion sites. No window is applied to the ~3,150-variant yield, unlike Amoeba's 1,000-item cap — see the crawler's own design spec for why. The sources-ship list below, the `_sync_stock`/success-criteria source mentions, the "Out of scope" line's further-source claim, and the Shopify-sites mentions (shared-logic bullet, future-direction bullet, sgrecordshop's own technical-grounding bullet) are all updated in place below. No changes to the data model, the orchestration loop, the API, or the frontend. Full design: [`2026-08-09-cleorecs-store-crawler-design.md`](../../specifications/shaping/2026-08-09-cleorecs-store-crawler-design.md).

**Amendment (2026-08-10, PR #92 review):** two further corrections, both made in place below. (1) The Cleopatra Records fallback artist is `"Various"`, not `"Various Artists"` — Discogs' entity name is `Various`, and `amazon.py`'s `_artist()` and `db.py`'s `_library_match_fragment` both compare against that exact string (`ebay_api.pick_matching_item` is a word-overlap ratio, not an exact-string check, and does not discriminate between the two spellings). (2) The `_sync_stock` and success-criteria lines described only `catalog` sources, but `_sync_stock` has loaded `catalog` **plus** `catalog_browser` since Angry Young and Poor introduced that kind; both lines are corrected in place.

**Amendment (2026-09-08, branch `claude/sacred-bones-vinyl-crawler-gufkfy`):** `iter_products()` no longer treats *any* falsy `products` field as pagination exhaustion. An empty **list** is exhaustion, and the only thing that is; a missing, null or retyped field now raises. The old reading was the one drift no crawler-side guard could see — from the crawler's side the walk simply ended, so on a later page `_sync_stock` replaced a store's whole snapshot with the partial prefix already yielded, and on the first page with an empty result no guard could distinguish from a store that sold out. Raising is inert by comparison: `_sync_stock` skips `replace_stock_items()` on a raise. This is the first change to `shopify_catalog.py`'s own behaviour since the 2026-08-04 Retry-After amendment, and it touches every Shopify-backed source; it was verified against the whole bundled Shopify crawler suite rather than only the source that prompted it. Found by Copilot in review on PR #332. The contract block and failure-behaviour paragraph under "Crawler plugin interface (catalog kind)" are updated in place. Full design: [`2026-09-08-sacred-bones-store-crawler-design.md`](../../specifications/shaping/2026-09-08-sacred-bones-store-crawler-design.md).

**Amendment (2026-08-10, branch `cleorecs-vinyl-crawler`):** the amendment just above corrected every claim it identified, but left four enumerative design-pattern lists unaddressed — row granularity, format-filter shapes, pre-order handling, and the `crawler_type` registration list (all under "Decisions" and just above "Crawler plugin interface (catalog kind)") — reasoning at the time that The Sound Garden was already missing from all four as a pre-existing gap predating this branch, so it was out of scope. On review, that reasoning doesn't hold: per this repo's CLAUDE.md, "a PR should not merge with known spec drift, even drift it didn't cause but exposed," and documenting the gap in the prior amendment's own report is what made it known. Both Cleopatra Records and The Sound Garden are added to all four lists below. Cleopatra Records slots into the existing patterns (multi-variant row producer; a new format-filter shape, since its product-type denylist's second layer excludes by product-level title-keyword rather than Triple B Records' variant-level negative filter; no pre-order override needed, but with no accepted gap, unlike the sites listed there, since its pre-orders already report `available: true`). The Sound Garden doesn't slot cleanly into any of them, since it has no Shopify product/variant hierarchy at all — each addition notes that explicitly rather than forcing a fit.

**Amendment (2026-08-23, branch `claude/spv-store-crawler-2mdf0z`):** the enumerative source lists below are a historical snapshot through 2026-08-10 and should be read as such. Many catalog sources have shipped since, starting with Turntable Lab on 2026-08-13 and running through Waterloo Records (#169), merged while this branch was open; each is documented in its own design doc under [`docs/specifications/shaping/`](../../specifications/shaping/) rather than as an amendment here, so treat those per-source docs as the live record of which sources exist. This amendment deliberately states no total: per `CLAUDE.md`, a count of things that change is not written down in this repo, and the repeated repair of the ones this document used to carry is exactly the churn that rule exists to stop. Nothing else in this spec — the orchestration loop, the plugin contract, the data model — is changed by any of those sources, including `spv.py`, added on this branch.

**Amendment (2026-09-01, branch `claude/stock-crawl-timeout-j15zt4`):** the fetch loop `iter_products()` is described as running below — jittered pre-request sleep, `raise_for_status()`, non-429 retries counted against `consecutive_failure_limit` (with `0` meaning fail-fast), immediate raise on the first 429 — is extracted into `catalog_http.get_with_retry()`, which `iter_products()` now calls per page. Semantics are unchanged, but the 2026-08-07 amendment's claim about The Sound Garden is superseded: every httpx-based catalog crawler that hand-rolled its own `client.get()` (Dark Descent Records, The Sound Garden, Dischord Records, Ripple Music, Asbestos Records, Jetglow Recordings) now fetches through the same helper, so "no retry loop at all / propagates on the first occurrence" no longer describes any of them, and the helper additionally stamps an explicit 30-second request timeout in place of httpx's 5-second default (a single slow TLS handshake past that default had cost Dark Descent an entire crawl on 2026-09-01). Full design: [`2026-09-01-stock-crawl-timeout-retry-design.md`](../../specifications/shaping/2026-09-01-stock-crawl-timeout-retry-design.md).

---

## Problem

The existing crawler system answers "what does site X charge for release Y in my collection?" — a per-release search driven by Playwright. There's a different question worth answering: "what's currently for sale at site X, regardless of whether I already own it?" These sources ship: Nuclear Blast (`shop.nuclearblast.com/collections/vinyl`), Century Media (`centurymedia.store/collections/vinyl`), Epitaph (`epitaph.com/collections/vinyl`), Rev HQ (`revhq.com/collections/vinyl`), Fat Wreck Chords (`fatwreck.com/collections/vinyl-1`), Jade Tree Records (`jadetree.store/collections/vinyl`), Deathwish Inc (`deathwishinc.com/collections/vinyl`), Equal Vision (`equalvision.com/collections/equal-vision-records`), Run For Cover (`runforcoverrecords.com/collections/vinyl-shop`), Secretly Store (`secretlystore.com/collections/vinyl`), Craft Recordings (`craftrecordings.com/collections/vinyl`), Relapse (`www.relapse.com/collections/vinyl`), Napalm Records (`napalmrecords.us/collections/vinyl`), Prosthetic Records (`shop.prostheticrecords.com/collections/vinyl`), Peaceville (`usa-peaceville.myshopify.com/collections/vinyl`), Season of Mist (`shopusa.season-of-mist.com/collections/vinyl`), 20 Buck Spin (`20buckspin.com/collections/vinyl`), Fearless Records (`store.fearlessrecords.com/collections/vinyl`), Flatspot Records (`flatspotrecords.com/collections/vinyl`), Rise Records (`riserecords.com/collections/all`), Triple B Records (`triplebrecords.net/collections/all`), Closed Casket Activities (`closedcasketactivities.com/collections/vinyl`), Big Scary Monsters USA (`usa.bsmrocks.com/collections/vinyl`), Kill Rock Stars (`killrockstars.com/collections/all`), Saddle Creek (`saddle-creek.com/collections/all`), Polyvinyl Record Co. (`polyvinylrecords.com/collections/vinyl`), Numero Group (`numerogroup.com/collections/vinyl`), Sub Pop Mega Mart (`megamart.subpop.com/collections/vinyl`), Fat Possum (`fatpossum.com/collections/vinyl`), Father/Daughter Records (`fatherdaughterrecords.com/collections/vinyl`), Temporary Residence Ltd (`temporaryresidence.com/collections/shop`), Pirates Press Records (`shop.piratespressrecords.com/collections/all`), The Sound Garden (`sgrecordshop.com`), and Cleopatra Records (`cleorecs.com/collections/vinyl-1`) — all full catalogs of in-stock vinyl, browsable independently of the user's Discogs collection/wishlist.
...
- **Checked for the hyphenated-artist-name collision found on Season of Mist and Pirates Press Records** — confirmed live against the full 9-product `/collections/vinyl-shop` catalog that one title triggers it: `"Ultra-Lite - Enjoy Your Time in the Sun LP"`, clipped by the old `\s*-\s*` split to artist `"Ultra"`. Fixed the same way as `seasonofmist.py`/`piratespressrecords.py`, requiring whitespace on at least one side of the splitting hyphen.
...
`shopusa.season-of-mist.com` is Shopify, and — like Peaceville — is the confirmed US-billed (USD via `cart.json`) storefront, distinct from the label's EUR-billed global store at `shop.season-of-mist.com`. Direct inspection found `vendor` is *always* `"Season of Mist - North America"`, for music and merch alike — the same landmine as Rev HQ/Deathwish Inc, but with zero exceptions rather than a rare fallback case. The real artist is embedded in the title as `"Artist - Album - Format"` (e.g. `"Windir - 1184 - DOUBLE LP GATEFOLD COLORED"`), which is parsed with a dash-split regex reusing Run For Cover's shape, modified to require whitespace on at least one side of the splitting hyphen (see the new bullet below for why).
...
- **Checked for the hyphenated-artist-name collision found on Season of Mist and Pirates Press Records** — confirmed live against the full 202-product `/collections/vinyl` catalog that one title triggers it: `"PAN-AMERIKAN NATIVE FRONT - LITTLE TURTLE'S WAR LP"`, clipped by the old `\s*-\s*` split to artist `"PAN"`. Fixed the same way as `seasonofmist.py`/`piratespressrecords.py`, requiring whitespace on at least one side of the splitting hyphen.
...
- **Checked for the hyphenated-artist-name collision found on Season of Mist and Pirates Press Records** — confirmed live against the full 1,391-product `/collections/vinyl` catalog that 8 titles trigger it, all clipped by the old `\s*-\s*` split (e.g. `"blink-182 - Dude Ranch"` → `"blink"`, `"Sleater-Kinney - Dig Me Out"` → `"Sleater"`, `"Wu-Tang Clan - Enter The Wu-Tang (36 Chambers)"` → `"Wu"`, `"The All-American Rejects - Move Along"` → `"The All"`, and `"C-Clamp"`/`"Michael Cormier-O'Leary"` similarly). Fixed the same way as `seasonofmist.py`/`piratespressrecords.py`, requiring whitespace on at least one side of the splitting hyphen.
...
### Pirates Press Records — `/collections/all` is cheaper and more complete than the store's own format-specific collections; a title-splitting regex needs care around hyphenated band names

`shop.piratespressrecords.com` is Shopify. Its nav exposes format-specific collections (`ppr-12-vinyl`, `ppr-7`, `ppr-10-vinyl`, plus a small `all-distro-titles` collection for vinyl the store distributes but doesn't press itself) rather than one collection holding all of its vinyl. An earlier draft of this crawler queried four of those collections and deduped the combined product stream by `id`, landing 500 unique products. Checking that against the storewide `/collections/all` feed (1,317 products across 6 pages) found it incomplete: 566 products storewide are vinyl-typed (`"Vinyl LP"`/`"Picture Disc"`), of which 560 have at least one available-or-preorder variant (the other 6 are sold out with no preorder tag — filtered out downstream regardless of which collection set finds them, so they're not part of either approach's count below). Of those 560 yieldable vinyl products, the four-collection approach reached only 496 — 64 real releases missed, including multi-color variants of in-print titles (The Drowns "Blacked Out", Territories "Colder Now", DeeCracks "True Nothing"), 7"s, and box sets. `/collections/all` reaches all 560 in fewer HTTP requests (7 vs. 9) and needs no dedup logic at all, so the shipped crawler uses it instead, joining Rise Records/Saddle Creek/Kill Rock Stars/Triple B Records' existing `/collections/all`-plus-product_type-filter shape.
  ...
  "products": [
    {
      ...
      "product_type": "Vinyl LP",
      "tags": ["45 Adapters", "Music", "new", "Pirates Press Records", "PPR12", "PPR391", "Vinyl LP"],
      "variants": [
...
- **Format filter is product-level, Equal Vision's shape**: `product_type` is confirmed to be either `"Vinyl LP"` or `"Picture Disc"` for every vinyl product storewide — both are vinyl, no CD/Cassette/merch contamination (the other 751 storewide products span 35 distinct non-vinyl `product_type` values: `"CD"`, `"T-Shirt"`, `"Pin"`, `"Cassette Tape"`, etc. — all cleanly excluded by the exact-match allowlist). Every vinyl product also has exactly one variant (confirmed across all 566), so — like Century Media/Epitaph — no per-variant filter is layered on top.
- **`vendor` = artist reliably, but the shared `strip_vendor_prefix` helper doesn't fire reliably, and a naive first-hyphen split is also wrong.** Every title is shaped `"{Artist} - {rest}"`, and `vendor` is confirmed to always be the correct artist — but the title's leading segment doesn't always exact-match `vendor` case-and-whitespace-for-case-and-whitespace: 58/566 titles mismatch (`vendor: "Crim"` vs. title `"CRIM - Blau Sang, Vermell Cel Black Vinyl LP"`; `vendor: "Reducers S.F"` vs. title `"Reducers S.F. - ..."`; `vendor: "Plizzken"` vs. title `"Plizzken -Do You Really Wanna Know? - ..."` with no space after the dash). Rather than widen the shared helper (used as an exact-prefix match by the other crawlers), this crawler splits the raw title on its own first `" - "` locally and keeps everything after as the display title, regardless of whether it matches `vendor`. That split must require whitespace on at least one side of the hyphen, not just any hyphen: two artists on this store have an internal hyphen with no surrounding space (`"A-100s"`, `"The Re-Volts"`), and a naive `\s*-\s*` split (tried first, and initially shipped) breaks on both — `"The Re-Volts - Wages Orange Vinyl 7\""` split into `"Volts"` / `"Wages Orange Vinyl 7\""` instead of `"The Re-Volts"` / `"Wages Orange Vinyl 7\""` — confirmed by testing both patterns against all 566 live vinyl titles; requiring whitespace on at least one side of the hyphen (but not necessarily both, since `"Plizzken -Do..."` above has none after it) fixes exactly those 5 mismatches and changes nothing else. 2/566 titles have no `" - "` at all (`"The Barstool Preachers Blatant Propaganda Black Vinyl LP"`, `"U.K.Subs C.I.D. Picture Disc Vinyl"`) — accepted as a known miss, same tradeoff as Deathwish Inc's quote-matching residual misses; those two fall back to the full raw title.
- **Pre-order tag is `"preorder"`** (lowercase, no hyphen — Century Media's spelling).
- Currency confirmed USD (`Shopify.currency = {"active":"USD",...}` on the live storefront).

---

## Decisions

- **Row granularity:** one row per in-stock **vinyl variant**, not one row per product. On Nuclear Blast, Rev HQ, Fat Wreck Chords, Deathwish Inc, Secretly Store, 20 Buck Spin, Triple B Records, Closed Casket Activities, Big Scary Monsters USA, Kill Rock Stars, Numero Group, Sub Pop Mega Mart, Fat Possum, Father/Daughter Records, Temporary Residence Ltd, and Cleopatra Records (196 of its 2,870 kept products are genuinely multi-variant, carrying pressing colours), a product with several in-stock vinyl variants produces that many rows. On Century Media, Epitaph, Jade Tree, Equal Vision, Run For Cover, Relapse, Napalm Records, Prosthetic Records, Peaceville, Season of Mist, Fearless Records, Flatspot Records, Rise Records, and Pirates Press Records this is moot in practice — every product has exactly one variant, or every variant is already vinyl — but the same one-row-per-variant model applies uniformly. Craft Recordings, Closed Casket Activities, and Big Scary Monsters USA are the exceptions with real multi-variant products that aren't all vinyl (see their filter notes below). **The Sound Garden has no product/variant hierarchy to begin with** — it isn't Shopify, so this bullet's per-variant-vs-per-product distinction doesn't apply to it; each scraped listing card is already its own catalog item with its own product ID (`pid`), and the only thing collapsed is a duplicate `pid` re-encountered across two of its 14 genre categories.
- **Format filter comes in several underlying shapes, and some sites combine or layer more than one.** *Positive regex* (Nuclear Blast, Rev HQ, Fat Wreck Chords, Secretly Store, Deathwish Inc, Kill Rock Stars, Saddle Creek, Numero Group, Sub Pop Mega Mart, Fat Possum, Father/Daughter Records, Temporary Residence Ltd): only variants whose title matches a vinyl-detecting regex are considered — each site's regex differs slightly (Rev HQ adds bare inch sizes like `"7\""`; Fat Wreck Chords/Secretly Store/Deathwish Inc/Numero Group also catch glued formats like `"2xLP"` that the narrower patterns miss; Father/Daughter Records needed a further widening — an optional trailing `s?` (`\b\d*x?lps?\b`) — to catch the plural `"LPs"`, which the bare `\b\d*x?lp\b` pattern misses because there's no word boundary between `"p"` and a trailing `"s"` — see each site's technical grounding above). Kill Rock Stars layers a *bundle-exclusion overlay* on top of the plain regex, since a confirmed-live vinyl+CD-bundle variant (`"LP + CD Bundle / X-Small"`) would otherwise false-positive on the `"LP"` substring alone. *Product-level type filter* (Equal Vision, Pirates Press Records; Rise Records via tags — see below; Saddle Creek and Temporary Residence Ltd each combine this with a positive regex, since `product_type == "Music"`/`"Albums"` still mixes other formats or a mistyped product in): `product_type` cleanly separates vinyl-eligible products from everything else before variants are even considered. *Negative filter* (Run For Cover excludes `"digital"`-matching titles; Craft Recordings excludes exact `"CD"`/`"Cassette"` titles; Closed Casket Activities and Big Scary Monsters USA exclude exact `"CD"`/`"Cassette"`/`"Digital"` titles) — used when the site's non-vinyl variants are the minority case and a positive filter would incorrectly exclude legitimate non-format variant titles (Craft Recordings' shirt sizes; Closed Casket Activities'/Big Scary Monsters USA's confirmed-live single-`"Default Title"`-variant vinyl products, which carry no format keyword at all). *Price/title-keyword filter* (20 Buck Spin): rather than a format signal, excludes zero/missing-price variants (a promo bundle) and title-keyword matches against known merch terms (a tote bag). *Tag-based product filter* (Rise Records): `product_type` is confirmed unreliable there, so a product must carry `"Music"` plus `"Vinyl LP"`/`"Vinyl 7"` tags to be included at all. *Product-type-exclusion plus narrow negative variant filter* (Triple B Records): real vinyl variants there carry no format keyword at all, so a positive regex is useless — a product is excluded entirely by a `product_type` denylist, then surviving products exclude only variants titled exactly `"CD"`/`"Digital"`. *Product-type denylist plus product-level merch-title-keyword filter* (Cleopatra Records): a product is dropped entirely if its `product_type` is in an eight-code denylist (`BND`/`BK`/`PS`/`PO`/`DVD`/`BR`/`TB`/`CD`), or if its title matches a merch-keyword regex (`poster`/`hardback book`/`tote bag`/`shirt`/`hoodie`/`sweater`/`bundle`) — this looks like Triple B Records' shape at a glance (both start with a `product_type` denylist), but the second layer differs: Triple B's excludes specific *variant* titles on products that already survived the denylist, while Cleopatra's excludes the whole *product* by a title-keyword match, the same technique 20 Buck Spin uses but without the price signal (Cleopatra has no zero-price promo listings to filter on). Century Media, Epitaph, Jade Tree, Relapse, Napalm Records, and The Sound Garden: no per-variant filter at all — confirmed live that their collections never mix formats (Sound Garden's 14 genre categories were checked across three samples and never showed a CD/cassette leaking in; its own `see-more-format` field is read for display only, not as a gate).
...
- **Full UI parity, not just columns:** Store also gets the artist sidebar (filter by artist, scoped to whatever's currently in `stock_items`), the search bar, and the list/tile view toggle — the same three UI elements `RecordBrowser` gives Collection and Wishlist. **(2026-08-27, branch `claude/mobile-optimized-web-qmv4u4`: the parity still holds below 768px — it is just parity in a different layout. Both browsers swap the sidebar for a sheet and the table for a card list there, from the same shared components. See [`2026-08-27-mobile-web-experience-design.md`](../../specifications/shaping/2026-08-27-mobile-web-experience-design.md).)** This needs a new `GET /api/stock/artists` endpoint (mirrors `GET /api/artists?scope=...`) and an `artist` filter param on `GET /api/stock` (mirrors `GET /api/releases?artist=...`). Tile view shows the cover image with artist/title below, linking out to the item's product page (`item.url`, whichever source it came from) instead of a Discogs release page. View-mode preference persists to `localStorage` under `collectionViewMode_instock`, following the same key pattern `RecordBrowser` uses per scope.
...
- **Owned-item cross-reference (added later, branch `store-tab-overlapping-filter`):** a filter dropdown sits left of the list/tile toggle, listing its three options in lexicographic order: a selectable "All" (the default, and how the user turns the filter back off), a selectable "Overlapping", and a disabled "Recommended" placeholder. Selecting Overlapping filters `stock_items` to rows whose artist matches (case-insensitive) an artist in the collection (`releases` where `in_collection = 1`), via a `LOWER(...) IN (SELECT LOWER(artist) ...)` subquery so the filter is enforced server-side and pagination/totals stay correct. No new table or join column — a query-time filter only. The artist sidebar (`GET /api/stock/artists`) takes the same `overlapping` flag and refetches whenever the dropdown changes, so the sidebar only lists artists that actually have matching rows under the active filter. The search box ANDs with whichever filter is active (both conditions are appended to the same `WHERE` clause), so search never bypasses Overlapping back to the full catalog. The chosen filter persists to `localStorage` under `stockFilter`, following the same pattern as `collectionViewMode_instock`.
  ...
  **Amendment (2026-08-08, branch `worktree-store-collection-split`):** the "Overlapping" dropdown option described above is removed, not just renamed — it's promoted to its own "Collection" nav tab (a `StockBrowser` instance with `scope="collection"`, always sending `overlapping=true` server-side; no dropdown renders there at all). Store's dropdown now offers only All/Recommended. The `overlapping` query param and its backend filtering logic (`get_stock_items`/`get_distinct_stock_artists`) are unchanged — only the frontend's Store-tab entry point to it moved. See [`2026-08-08-store-collection-split-design.md`](../../specifications/shaping/2026-08-08-store-collection-split-design.md) for the current design, which also adds cross-site comparison-price rows so a page can render more table rows than items.

  **Amendment (2026-08-09, branch `worktree-collection-price-paid`):** the "Columns, left to right" list above is stale in two ways. The `Price` column (hyperlink to the store/comparison listing) is relabeled `Cost` in both Store and Collection. Collection scope only gains a second, new `Price` column — the matched Discogs collection item's custom-field "price paid" value (`catalog.discogs_price`, the same value the Discogs tab already shows), sorted via `sort=discogs_price`. Store scope never shows this second column. See [`2026-08-09-collection-price-paid-design.md`](../../specifications/shaping/2026-08-09-collection-price-paid-design.md).

  **Follow-up (2026-08-09, branch `worktree-library-price-paid`):** the column and its sort key are unchanged, but its source is no longer `catalog.discogs_price` — that column is dropped. It now reads the calling user's own `library_items.price_paid`, aliased back to `discogs_price` so neither the API shape nor the frontend changed.
  ...
  **Amendment (2026-08-16, branch `store-saved-items`):** "Store's dropdown now offers only All/Recommended," above, is stale — `Saved` was added for a per-user "save for later" bookmark, unrelated to `Recommended`/`library_scope`. See [`2026-08-16-store-saved-items-design.md`](../../specifications/shaping/2026-08-16-store-saved-items-design.md).

  **Amendment (2026-08-26, branch `claude/store-overlapped-artist-filter-i3cp7i`):** the dropdown gained `Overlapped`, so it now offers All/Recommended/Saved/Overlapped. Read alongside the 2026-08-08 amendment above it needs care: the option that amendment records being *removed* was the release-level "Overlapping" (artist **and** title, promoted to what is now the Track tab's Collection filter), and that one is still gone. `Overlapped` matches at *artist* level — any in-stock record by an artist the user collects, whether or not they own that record — so it is a filter the Store tab never previously had, not the old one returning. See [`2026-08-26-store-overlapped-artist-filter-design.md`](../../specifications/shaping/2026-08-26-store-overlapped-artist-filter-design.md).
- **Artist casing (added later, branch `store-tab-overlapping-filter`):** `replace_stock_items` applies Python's `str.title()` to `item["artist"]` before insert — the single write path for `stock_items` (called from `CrawlManager._sync_stock`), so every catalog crawler gets normalized casing for free without per-crawler changes. Known tradeoff: `.title()` mangles some real band-name stylings (all-caps names like "NAILS" become "Nails"; it also mis-cases text after apostrophes) — accepted for consistent, predictable display over exact stylization fidelity. Applies at crawl time only; existing rows keep whatever casing they had until the next sync.
  ...
  **Amendment (2026-08-07, branch `worktree-fix-title-case-artist`, PR #50):** `str.title()` also mangles digit/letter boundaries — treating each as a new word — so "13th Floor Elevators" became "13Th Floor Elevators", and its `[A-Za-z]+`-based first fix regressed non-ASCII names (accented letters like the "ö" in "björk" split the name into two word runs, producing "BjöRk"). `replace_stock_items` now runs the raw artist string through a new `_title_case_artist()` helper in `db.py` instead of calling `.title()` directly: it walks the string character-by-character using `str.isalpha()` (Unicode-aware, unlike an ASCII-only regex) and only capitalizes a letter run's first character when it isn't immediately preceded by a digit, lowercasing every other letter — otherwise matching `.title()`'s word-by-word behavior, including the ALL-CAPS and post-apostrophe tradeoffs above. Because this changes the display casing for some artists (digit-prefixed ones), `item_key` is deliberately *not* recomputed from the corrected casing — it still hashes the legacy `item["artist"].title()` value, so existing `stock_item_judgments` rows (which join on `item_key`) don't silently orphan for any artist whose display casing changed under this fix.

  **Amendment (2026-08-07, branch `fix-stock-artist-casing`):** revisited after the Pirates Press Records crawler's final code review flagged a worse case than the ones above — that store has hyphenated, no-natural-single-capital names like "A-100s", and word-by-word title-casing turns those into "A-100S" (capitalizing the letter right after the hyphen), which reads as a more obviously broken mangling than the ALL-CAPS case, not just a stylization-fidelity tradeoff. (`_title_case_artist()`'s digit-adjacency check above already happens to leave a same-case name like "a-100s" or "A-100S" correct, since the "s"/"S" directly follows a digit — but it does nothing for the actually-reported, already-mixed-case "A-100s".) A new `normalize_artist_casing()` helper now gates `_title_case_artist()` behind a same-case check: it only title-cases input that's entirely one case to begin with (`artist.isupper()` or `artist.islower()`); anything already mixed-case — hyphenated names like "A-100s", already-correct stylings like "The Beatles", or deliberately-lowercase stylings like "of Montreal" — passes through untouched instead of being reformatted into rigid word-by-word title case. `replace_stock_items` calls `normalize_artist_casing()` for the stored `artist` value; `item_key` is unaffected by this change and keeps hashing the legacy `item["artist"].title()` value per the amendment above. All-caps and all-lowercase input is otherwise unchanged (e.g. "NAILS" still becomes "Nails"); only the mixed-case path is new. Affects all catalog crawlers via the same shared write path.

  **Amendment (2026-08-08, branch `store-title-source-casing`):** same problem, same fix, extended to `title`. Some stores also return release titles in ALL CAPS. `_title_case_artist()` was renamed to `_title_case_words()` (it was never artist-specific — it's a plain string transform) and a parallel `normalize_title_casing()` was added, applying the identical same-case gate to `item["title"]`. `replace_stock_items` now calls both `normalize_artist_casing()` and `normalize_title_casing()` when building each row; `item_key` continues hashing the raw, unnormalized `item["title"]` (alongside the already-unnormalized legacy artist casing), so existing `stock_item_judgments` rows don't orphan for any item whose displayed title casing changed under this fix.

  **Amendment (2026-08-14, branch `artist-name-normalization`):** the same-case gate above means write-time normalization can't reconcile *sources* that disagree on capitalization — a Shopify vendor field reading "Jets to Brazil" is mixed-case, so it passes through untouched, while another store's "JETS TO BRAZIL" becomes "Jets To Brazil", and `catalog` (Discogs, stored verbatim) may hold a third spelling. Since `SELECT DISTINCT s.artist` and the `artist` filter were both case-sensitive, that showed up as one artist listed twice in the sidebar with the filter splitting their records. Display casing is now resolved at *read* time by `db.canonical_artist_labels()`, which picks one label per case-folded artist — the `catalog` casing when the artist appears there, else the commonest `stock_items` casing — and `get_distinct_stock_artists`/`get_stock_items`/`get_distinct_artists`/`get_library_releases` all label their rows through it and match the `artist` filter case-insensitively. `replace_stock_items` is unchanged, and `item_key` still hashes the legacy raw `.title()` value. Because that label is derived from live row counts, a sync can change it while the artist is selected in the sidebar, so both browsers reconcile the active selection against each refetched artist list (`frontend/src/views/artistSelection.ts`) rather than leaving an unhighlighted, invisible filter. See [`2026-08-14-artist-casing-canonicalization-design.md`](../../specifications/shaping/2026-08-14-artist-casing-canonicalization-design.md).

  **Amendment (2026-08-16, branch `claude/artist-the-sorting-5a0b95`):** `canonical_artist_labels()`'s picked label is now also run through a "The X" -> "X, The" fold — "The Beatles" displays as "Beatles, The", matching the library-catalog convention some stores already spell directly, and the two forms collapse to one sidebar entry instead of two. The `artist` filter in `get_library_releases`/`get_stock_items` folds the same way on both sides, so a click on the folded label still finds rows literally stored as "The X". See [`2026-08-16-the-suffix-artist-display-design.md`](../../specifications/shaping/2026-08-16-the-suffix-artist-display-design.md).

  **Amendment (2026-08-23, branch `claude/band-the-grouping-artists-752bf7`):** a third spelling joins that group — the bare name with no article ("Beatles"), which some stores crawl instead of either "The Beatles" or "Beatles, The". It carries no marker to fold on, so `canonical_artist_labels()` resolves it with a lookup phase, run before the casing rule, that asks whether `catalog` or `stock_items` holds a "The"/comma-form spelling of the same name; if so the bare input takes that group's label, otherwise it falls through unchanged and a genuinely article-less artist is unaffected. The `artist` filter in `get_library_releases`/`get_stock_items` moved to `_artist_sort_sql`'s article-stripped key so a click on "Beatles, The" returns all three raw spellings. `search=` deliberately did not change and does not match a bare-stored row against the full label — a known asymmetry recorded in that design's "Out of scope". See [`2026-08-22-bare-form-artist-fold-design.md`](../../specifications/shaping/2026-08-22-bare-form-artist-fold-design.md).

  **Amendment (2026-09-13, branch `claude/friendly-hamilton-vymyyk`):** the same group now also spans the punctuation two sources disagree on — "&" against "and", "-" against " " — so Discogs' "Blink-182" and a store's "Blink 182", or "Hall & Oates" and "Hall and Oates", are one sidebar entry holding all of their records rather than two entries splitting them. `_artist_punct_fold_sql` wraps the column before either the grouping key or `_artist_sort_sql` computes anything, so `canonical_artist_labels()`, the `artist=` equality filter, the artist sort, the `Overlapped` artist match and the `Cheapest` partition all inherit it at once. The displayed label is still a spelling some source actually wrote — whichever won the existing frequency vote — and `search=` again deliberately did not change. The artist expression indexes are renamed with the expression (`*_the_fold_idx`, `*_bare_fold_idx`, `stock_items_cheapest_fold_idx`), since `CREATE INDEX IF NOT EXISTS` cannot redefine one in place. See [`2026-09-13-artist-punctuation-fold-design.md`](../../specifications/shaping/2026-09-13-artist-punctuation-fold-design.md).
- **Pre-order handling is per-site, and several sites get none — plus one site with an unstructured signal instead of a tag, one site where the signal exists but is unreachable through the endpoint used, and some sites matched via substring/regex instead of exact tag equality.** Nuclear Blast/Century Media/Epitaph/Fat Wreck Chords/Deathwish Inc/Secretly Store/Equal Vision/Craft Recordings/Relapse/Napalm Records/Prosthetic Records/Peaceville/Fearless Records/Rise Records/Big Scary Monsters USA/Kill Rock Stars/Numero Group/Father/Daughter Records/Temporary Residence Ltd/Pirates Press Records all tag pre-order products in their `tags` array with one exact, stable tag string (spelled `"pre-order"`, `"preorder"`, `"pre-order"`, `"preorder"`, `"Pre-Order"`, `"Pre-Order"`, `"preorder"`, `"_preorder"`, `"preorder"`, `"preorder"`, `"Pre-Orders"`, `"preorder"`, `"preorder"`, `"preorder"`, `"Pre-order"`, `"Preorder"`, `"Street Date"`, `"Pre-order"`, `"Flag_Pre-Order"`, `"preorder"` respectively — confirmed via direct fetch; Craft Recordings' leading-underscore spelling, Prosthetic Records' plural form, Numero Group's non-obvious "Street Date" wording, and Temporary Residence Ltd's underscore-flag convention are each unique to that site; Pirates Press Records' spelling matches Century Media's), but individual variant `available` flags on a pre-order product are inconsistent — some variants show `available: true`, others `false`, even though the whole release is purchasable. Flatspot Records uses two tag forms at once — a generic `"pre-order"` (hyphenated) and a dated `"Pre-Order MM-DD-YY"` — matched via a `^pre-order` regex rather than `has_tag`'s exact match, since the dated form would otherwise be missed. **Closed Casket Activities and Polyvinyl Record Co. instead need a substring search** (`pre-?order` matched anywhere within any tag string, case-insensitively) — Closed Casket Activities has two coexisting tag conventions (`"__label:Pre-Order"` and bare `"preorder"`), and Polyvinyl Record Co. bakes the release name into the tag itself (e.g. `"AF - Live in LA Pre-Order"`), so no single canonical tag string exists on either site. **Season of Mist is the one exception with a signal at all but no tag** — pre-order status only appears in free-text `body_html`, detected via a regex search instead of `has_tag` (see its technical grounding above; this is the most fragile pre-order mechanism in the batch, since it depends on the label's marketing copy wording rather than a stable field). **Sub Pop Mega Mart is a different kind of gap**: a `"pre-order"` tag genuinely exists in this store's data, but the crawler's chosen endpoint (`/collections/vinyl`, picked because it's cleanly apparel-free) confirmed-live never returns pre-order-tagged products at all — so no override is implemented, not because the signal doesn't exist, but because it's unreachable through the endpoint used. For any product carrying a site's pre-order signal, all of its vinyl variants are included regardless of `available`, and the title gets a `" (Pre-Order)"` suffix. Rev HQ, Jade Tree, Run For Cover, 20 Buck Spin, Triple B Records, Saddle Creek, and Fat Possum have no confirmed structured pre-order signal at all (see their technical grounding above; Saddle Creek's and Fat Possum's "none found" are each confirmed against a substantial sample, not a handful of products), so they get no override — just the plain `available == true` filter, with the accepted gap that a genuine pre-order on any of those sites could be excluded. **Cleopatra Records and The Sound Garden also implement no override, but neither carries that accepted gap.** Cleopatra Records' pre-orders (30 confirmed live) already report `available: true`, so the plain filter includes them with nothing missed. The Sound Garden's stock gate is presence-of-price rather than `available`, and pre-order-priced items pass it the same as any other in-stock item regardless of their shipping-status text (`"In Stock"`, `"Special Order..."`, etc., none of which gate the result) — its own "Pre-Orders" featured category is skipped as a crawl target, but only because it's a near-total subset of the 14 genre categories that are crawled, not because pre-order items are excluded from those.
...
### `backend/crawlers/piratespressrecords.py`

- Uses `iter_products(base_url, "all")` — note the non-standard collection ("vinyl"-named collections exist but none holds the full catalog; see technical grounding above for why `/collections/all` was chosen over them) — plus `has_tag` and `resolve_cover_image`; does **not** use `strip_vendor_prefix` — title parsing is a local dash-split instead (see below).
- For each product: return no items at all unless `product_type` is `"Vinyl LP"` or `"Picture Disc"` (this site's product-level filter, Equal Vision's shape — `/collections/all` also carries 35 other `product_type` values, all merch/CD/Cassette, all cleanly excluded). `vendor` → artist directly. Display title is everything after the title's own first `" - "` (a local regex requiring whitespace on at least one side of the hyphen, not `strip_vendor_prefix`, since `vendor` doesn't reliably exact-prefix-match the title here, and a plain `\s*-\s*` split would also incorrectly break on the two artists whose own name contains an unspaced hyphen — see technical grounding above); falls back to the full raw title on the rare title with no `" - "` separator at all.
- For each variant (of an already-vinyl-typed product, and there's always exactly one): skip unless `available` is true or `has_tag(product, "preorder")`. Pre-order items get `" (Pre-Order)"` appended to the title. `format` is always `"Vinyl"`. `cover_image_url` via `resolve_cover_image(product, variant)`. Yields `{"artist", "title": f"{title}[ (Pre-Order)]", "format": "Vinyl", "price": float(variant["price"]), "currency": "USD", "url": f"{base_url}/products/{handle}", "cover_image_url"}`.

### `backend/crawlers/sgrecordshop.py`

- Does not use `shopify_catalog.py` at all — the first `catalog`-type crawler that doesn't, since this site (FieldStack Omni, not Shopify) has no `products.json`-style JSON endpoint. Fetches via a bespoke two-step flow: `GET` a category page, scrape a `SearchId` GUID out of its inline `<script>`, then `GET /gsrp/{page}?{querystring}` with that GUID in an `X-Search-Guid` header, once per category (`SearchId` is reused across all of that category's pages, not re-scraped per page) across 14 hardcoded "by genre" category paths.
- Parses the returned HTML fragment with regex (no HTML-parsing library exists in this codebase) rather than consuming JSON or querying a live DOM (the two techniques every other `catalog`/`catalog_browser` crawler uses) — `artist` comes from the listing's `product-title` span (confirmed reliably untruncated), and `title` is recovered by stripping that same artist text as a literal prefix off the anchor's untruncated `title="Artist/Title@variant"` attribute, rather than splitting on `/` positionally (neither a first-`/` nor last-`/` split is correct for all observed cases — see the crawler's own design spec for the two live collisions that ruled each out).
- Skips any item rendered with a `product-variant-unavailable` marker (no price at all) — the only sold-out signal this crawler relies on. Deduplicates every yielded item by product id (`pid`, parsed from the listing URL) across the whole crawl, including across categories, since category overlap wasn't exhaustively ruled out. `format` comes from the listing's own format text (`"Vinyl LP"`, `"Vinyl 12\""`, etc.) rather than a regex guess. Yields `{"artist", "title", "format", "price": float, "currency": "USD", "url": f"{base_url}/p/{pid}/", "cover_image_url"}` — no `condition` field, since used-condition items are out of scope entirely (the 4 "used records" categories are never crawled).
- Applies `crawl_delay_seconds` jitter before every request (category-page GET and every `/gsrp/` page GET) but does **not** read `consecutive_failure_limit` — there's no retry loop at all; any non-2xx from either endpoint propagates on the first occurrence. This is a real reduction in resilience relative to `iter_products()`'s retry-until-the-limit behavior for the Shopify sites, not an equivalent alternative — accepted for now since no failure mode was observed to design around during live investigation; 80 sequential requests per crawl (14 categories × up to several pages each) makes a single transient failure costlier here than for any Shopify site, most of which complete in 1-3 requests total.

Full design: [`2026-08-07-sgrecordshop-store-crawler-design.md`](2026-08-07-sgrecordshop-store-crawler-design.md).

### `backend/crawlers/cleorecs.py`

- Uses `iter_products(base_url, "vinyl-1")` and `resolve_cover_image` from the shared module; does **not** use `strip_vendor_prefix` or `has_tag` — `vendor` is the imprint (Cleopatra Records, Purple Pyramid, Deadline Music, and others) on every product, never the artist, so artist comes entirely from title-parsing, and this store's pre-orders already report `available: true` so no override is needed.
- For each product: return no items at all if `product_type` is in `{BND, BK, PS, PO, DVD, BR, TB, CD}`, or if the title matches a merch-keyword regex (`poster|hardback book|tote bag|shirt|hoodie|sweater|bundle`) — `product_type` alone is correct on today's data, but the title check backs it up the same way 20 Buck Spin's does. Artist/album split is a local dash-split on the title with trailing parentheticals stripped first (a regex accepting `-`/`–`/`—` with whitespace on at least one side, this spec's now-standard hyphenated-artist-name-safe shape), falling back to `artist = "Various"` with the full original title on the 5.6% of products with no separator — bare `"Various"`, Discogs' own entity name, because `amazon.py`'s `_artist()` and `db.py`'s `_library_match_fragment` both compare against that exact string and `"Various Artists"` satisfies neither.
- For each variant: skip unless `available` is true. Title is `album` when the variant title is `"Default Title"`, else `f"{album} — {variant_title}"` — a placeholder guard `subpopmegamart.py`/`twentybuckspin.py` lack, needed here since 2,650 of 3,151 live variants carry the `"Default Title"` placeholder. `format` is always `"Vinyl"`, including for `SP` 7" products. `cover_image_url` via `resolve_cover_image(product, variant)`.
- No window on the ~3,150-variant yield, unlike Amoeba's 1,000-item cap — see the crawler's own design spec for the accepted queue-fan-out consequence.

Full design: [`2026-08-09-cleorecs-store-crawler-design.md`](../../specifications/shaping/2026-08-09-cleorecs-store-crawler-design.md).
...
- `_sync_stock()` loads **all enabled catalog crawlers of both kinds** — `get_enabled_crawlers("catalog") + get_enabled_crawlers("catalog_browser")`, so every `catalog` site this spec enumerates plus the `catalog_browser` ones (Angry Young and Poor, Amoeba), plus any future one — the loop is data-driven off the `crawlers` table, not hard-coded. For each: calls `crawl_catalog()`, replaces that crawler's `stock_items` rows, and broadcasts progress.
- New SSE events on the existing `/api/crawl/stream` channel (no new stream): `stock_sync_started`, `stock_sync_progress` (`{synced, source}`), `stock_sync_complete` (`{synced}`), `stock_sync_error` (`{error, source}`). **(2026-08-02 amendment:** `stock_sync_aborted` (`{error, sources}`) added — fires instead of `stock_sync_complete` when two consecutive catalog crawlers fail with HTTP 429 in the same run; a lone 429 that doesn't reach that threshold is logged at `WARNING` and does not broadcast `stock_sync_error` at all, since it's an expected, handled condition rather than a failure.) **(2026-08-09 amendment:** `stock_sync_source_started` (`{source}`) fires before each crawler runs, and `stock_sync_page_fetched` (`{source, page, page_count}`) fires once per listing page a crawler fetches, so the bottom status bar names the store and its live page progress instead of sitting on a generic message for the whole run. Crawlers report pages through `crawl_progress.report_page()`, whose reporter `_run_catalog_crawler` installs in a `contextvars` variable for the duration of one crawler's `crawl_catalog()` — async generators run in their caller's context, so `shopify_catalog.iter_products()` and the hand-rolled paging loops reach it without threading a callback through every plugin.) **(2026-08-25 amendment:** `stock_sync_detail_progress` (`{source, done, total, label}`) added, reported through a second `contextvars` reporter, `crawl_progress.report_detail()`, installed by `_run_catalog_crawler` the same way. `report_page()` fires once per *listing* page, which for a two-phase crawler is a whole page's worth of paced detail fetches apart -- tens of minutes on `dischordrecords.py` -- so a two-phase crawler reports its detail fetches too. Both reporters, and `stock_sync_source_started`, now write an INFO log line as well as broadcasting: the status bar shows only the latest event and is gone on reload, so the Log Viewer never named the source being crawled. See [`2026-08-25-catalog-crawl-progress-visibility-design.md`](../../specifications/shaping/2026-08-25-catalog-crawl-progress-visibility-design.md).)
- `db.py` additions: `replace_stock_items(conn, crawler_id, items)` (delete-then-insert in one transaction), `get_stock_items(conn, search=None, sort="artist", order="asc", page=1, per_page=50)` — sortable by `artist`, `title`, `format`, or `price`. **(2026-09-02 note:** this is the original design-time signature, and both it and the sort list have been overtaken by later designs. Read the function for the current shape rather than any inventory written here. The response shape has moved on too: see [`2026-08-08-store-collection-split-design.md`](../../specifications/shaping/2026-08-08-store-collection-split-design.md) for the comparison rows and the `Cost` sort's flat ordering.)
...
- `GET /api/stock/artists` — distinct artists currently in `stock_items`, for the sidebar, plus the same `overlapping` boolean filter as `GET /api/stock`. Mirrors `GET /api/artists`. (Distinct case-insensitively as of 2026-08-14, one canonically-cased label per artist — see the casing amendments above.)
...
- `App.tsx`: `View` union gains `'instock'`; new nav button "Store" (originally "In Stock") next to Wishlist. SSE handler gains cases for `stock_sync_started/progress/complete/error` (2026-08-02 amendment: plus `stock_sync_aborted`; 2026-08-09 amendment: plus `stock_sync_source_started` → "Syncing in-stock catalog… {source}" and `stock_sync_page_fetched` → "Syncing in-stock catalog… {source} fetched page {page}, {page_count} products", singular "product" when `page_count` is 1; 2026-08-25 amendment: plus `stock_sync_detail_progress` -> "Syncing in-stock catalog... {source} {label} - {done}/{total} detail pages", singular "detail page" when `total` is 1 -- deliberately not "releases" or "products", since one detail fetch is a release on `dischordrecords.py` and a variable product on `darkdescentrecords.py`), reusing the existing bottom status bar (`syncMessage`/`syncing`) rather than a new UI element.
...
- Any further catalog source beyond those built here (the orchestration loop and the `catalog` crawler-plugin contract support it structurally, but no additional crawler is being written now). This line used to name a specific next-source ordinal and had to be corrected three times as unplanned sources shipped; it is deliberately count-free now.
...
- "Refresh Stock Now" populates the Store tab with in-stock vinyl variants from every source the stock sync enumerates (the `catalog` sites this spec covers plus the `catalog_browser` ones), each priced and linked to its product page, with the correct source shown per row.
- Pirates Press Records rows never include a hyphenated artist name (e.g. "The Re-Volts", "A-100s") with its first syllable clipped off by the title-splitting regex.
...
- (Superseded by the 2026-08-08 amendment above — Overlapping is no longer a Store-tab dropdown option; the four bullets below describe pre-`store-collection-split` behavior and now apply to the Collection tab instead.)
