...
**Amendment (2026-08-23, branch `claude/spv-store-crawler-2mdf0z`):** every source count in this document is stale, and this amendment does not repair them one by one. The last count correction above (36, in the 2026-08-10 PR #92 review note) predates fifteen further catalog sources, none of which amended this spec — starting with Turntable Lab on 2026-08-13 and running through Waterloo Records (#169), merged while this branch was open (fourteen `catalog`-type, plus SideOneDummy Records as a third `catalog_browser`), each documented in its own design doc under [`docs/specifications/shaping/`](../../specifications/shaping/) rather than as an amendment here. Recounted directly against the tree with the same greps Amendment 11 of [`2026-07-06-store-recommended-filter-design.md`](2026-07-06-store-recommended-filter-design.md) used, as of 2026-08-24: **48** `catalog`-type plus **3** `catalog_browser`-type, so `_sync_stock` crawls **51** sources, not 36. That figure is a snapshot and will rot the next time a crawler lands — it is recorded to show the scale of the drift, not as a number to rely on. Treat the per-source design docs as the live record of which sources exist; the enumerative lists below are a historical snapshot through 2026-08-10. Nothing else in this spec — the orchestration loop, the plugin contract, the data model — is changed by any of those fifteen.
...
- `App.tsx`: `View` union gains `'instock'`; new nav button "Store" (originally "In Stock") next to Wishlist. SSE handler gains cases for `stock_sync_started/progress/complete/error` (2026-08-02 amendment: plus `stock_sync_aborted`; 2026-08-09 amendment: plus `stock_sync_source_started` → "Syncing in-stock catalog… {source}" and `stock_sync_page_fetched` → "Syncing in-stock catalog… {source} fetched page {page}, {page_count} products", singular "product" when `page_count` is 1), reusing the existing bottom status bar (`syncMessage`/`syncing`) rather than a new UI element.
