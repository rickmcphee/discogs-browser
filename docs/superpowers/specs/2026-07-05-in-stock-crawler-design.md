...
- `App.tsx`: `View` union gains `'instock'`; new nav button "Store" (originally "In Stock") next to Wishlist. SSE handler gains cases for `stock_sync_started/progress/complete/error` (2026-08-02 amendment: plus `stock_sync_aborted`), reusing the existing bottom status bar (`syncMessage`/`syncing`) rather than a new UI element.
