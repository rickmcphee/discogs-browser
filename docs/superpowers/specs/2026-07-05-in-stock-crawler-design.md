...
**Amendment (2026-08-10, PR #92 review):** two further corrections, both made in place below. (1) The Cleopatra Records fallback artist is `"Various"`, not `"Various Artists"` — Discogs' entity name is `Various`, and `amazon.py`'s `_artist()`, `db.py`'s `_library_match_fragment` and `ebay_api.pick_matching_item` all key on that exact string. (2) The `_sync_stock` and success-criteria lines counted only `catalog` sources, but `_sync_stock` has loaded `catalog` **plus** `catalog_browser` since Angry Young and Poor introduced that kind — thirty-six sources today, not thirty-four. Every other count in this document is a `catalog`-only count and remains correct as written.
