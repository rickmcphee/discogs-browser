...
- New SSE events on the existing `/api/crawl/stream` channel (no new stream): `stock_sync_started`, `stock_sync_progress` (`{synced, source}`), `stock_sync_complete` (`{synced}`), `stock_sync_error` (`{error}`). **(2026-08-02 amendment:** `stock_sync_aborted` (`{error, sources}`) added — fires instead of `stock_sync_complete` when two consecutive catalog crawlers fail with HTTP 429 in the same run.)
