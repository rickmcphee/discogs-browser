...
- `GET /api/stock` — search/sort/paginate `stock_items` joined to `crawlers.site_name` as `source`, plus an `artist` filter. Mirrors `get_releases`'s shape (search across artist/title, filter by artist, sort by artist/title/format/price, paginated).
- `GET /api/stock/artists` — distinct artists currently in `stock_items`, for the sidebar. Mirrors `GET /api/artists`.
...
- "Refresh Stock Now" populates the In Stock tab with in-stock vinyl variants from all four sources, each priced and linked to its product page, with the correct source shown per row.
