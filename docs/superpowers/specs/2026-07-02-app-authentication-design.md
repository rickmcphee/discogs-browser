...
**Amendment (2026-07-26): this entire design is retired.** The single-owner password+TOTP system described below — `auth_core.py`, `reset_owner.py`, the `owner`/`session` SQLite tables, `/api/auth/login`/`/setup`/`/setup/verify`/`/change-password`/`/reset-totp`/`/regenerate-recovery-codes` — has been deleted outright as part of a multi-tenant pivot. The app now uses Discogs OAuth 1.0a login, gated by invite code for new accounts. See [`docs/superpowers/specs/2026-07-26-multi-tenant-architecture-design.md`](2026-07-26-multi-tenant-architecture-design.md) and [`docs/superpowers/specs/2026-07-26-discogs-oauth-auth-design.md`](2026-07-26-discogs-oauth-auth-design.md) for the current design. Everything below is left as historical record of the retired system, not current behavior — not rewritten in place, per this repo's convention of additive amendments over history rewrites.
