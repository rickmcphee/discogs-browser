...
**Amendment (2026-08-10, branch `worktree-collection-wishlist-filter`):** tab-label pointer only — no behavior described below changes (it was all removed anyway, per the 2026-07-31 amendment). This document's "Collection tab" means the `RecordBrowser` tab over the Discogs **collection**, and its "Wishlist tab" the `RecordBrowser` tab over the **wantlist**. Under the current names those are **Collection** (the label round-tripped: renamed to "Discogs" on 2026-08-08, back to "Collection" on 2026-08-10) and **Wantlist**. Note that "Collection" here never means the store/library intersection tab, which is now called **Track**. The per-tab key pattern the amendment below describes outlived the filter itself: `` collectionFilter_${scope} `` is gone with the rest of the Plex feature, but `` collectionViewMode_${scope} `` remains, and its `scope` values moved with the labels (`'wishlist'` → `'wantlist'`, and `'collection'` → `'discogs'` → back to `'collection'`), so the key names it produces are not the ones written below. See [`2026-08-10-collection-wishlist-filter-design.md`](../../specifications/shaping/2026-08-10-collection-wishlist-filter-design.md) for the mapping.
