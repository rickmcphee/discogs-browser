...
**Amendment (2026-08-29, branch `claude/discogs-price-tracking-matches-vs426m`):** the amendment below frames `empty_result_is_expected` as a single-shop concession, on the reasoning that "a real Discogs release absent from a near-universal marketplace is evidence something is wrong." The Discogs marketplace crawler now sets it too, which is not a contradiction of that reasoning but a narrowing of it: the inference from emptiness to breakage is only needed while a crawler *cannot tell the two apart*, and it is exactly as wrong for a marketplace crawler that can. That crawler had been returning `[]` for pages it failed to parse, so the breaker was reading a parse failure as a marketplace miss and cooling the site off — while the write path, reading the same `[]`, erased prices it had already found. It now raises on a page it could not read, which is the signal item 9 says the breaker needs, and its remaining empty results are confirmed rather than suspected. See the 2026-08-29 amendment to [`2026-07-08-collection-price-crawlers-design.md`](2026-07-08-collection-price-crawlers-design.md).

**Amendment (2026-08-28, branch `claude/store-crawler-activity-missing-1tybli`):** item 8's release-target half — an empty result on a *release* row counts as a failure — is no longer unconditional. Its rationale is that a real Discogs release absent from a near-universal marketplace is evidence something is wrong, and that reasoning does not survive contact with a crawler for a single shop: Waterloo Records stocks a fraction of any given library, so a run of releases it does not carry is its ordinary healthy behaviour. Left as it was, `consecutive_failure_limit` such releases in a row would trip the breaker and cool off a site that answered every request correctly, and a user whose library overlaps the store thinly would see it crawl only in short bursts. *(2026-08-29: Waterloo Records has since reverted to a catalog crawler and no longer declares the attribute — see the 2026-08-29 amendment to [`2026-08-24-waterloo-records-crawler-design.md`](../../specifications/shaping/2026-08-24-waterloo-records-crawler-design.md). The mechanism stands, and `discogs_marketplace` still declares it per the amendment above.)*
